(self["webpackChunk_mattermost_webapp"] = self["webpackChunk_mattermost_webapp"] || []).push([[31],{

/***/ 25527:
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

"use strict";
// ESM COMPAT FLAG
__webpack_require__.r(__webpack_exports__);

// EXPORTS
__webpack_require__.d(__webpack_exports__, {
  "default": () => (/* reexport */ global_threads)
});

// EXTERNAL MODULE: ./node_modules/lodash/isEmpty.js
var isEmpty = __webpack_require__(41609);
var isEmpty_default = /*#__PURE__*/__webpack_require__.n(isEmpty);
// EXTERNAL MODULE: ./node_modules/core-js/modules/web.dom-collections.iterator.js
var web_dom_collections_iterator = __webpack_require__(33948);
// EXTERNAL MODULE: ./node_modules/core-js/modules/es.promise.js
var es_promise = __webpack_require__(88674);
// EXTERNAL MODULE: ./node_modules/react/index.js
var react = __webpack_require__(67294);
// EXTERNAL MODULE: ./node_modules/react-intl/lib/src/components/useIntl.js
var useIntl = __webpack_require__(86896);
// EXTERNAL MODULE: ./node_modules/react-router-dom/esm/react-router-dom.js
var react_router_dom = __webpack_require__(73727);
// EXTERNAL MODULE: ./node_modules/react-router/esm/react-router.js + 1 modules
var react_router = __webpack_require__(5977);
// EXTERNAL MODULE: ./node_modules/react-redux/es/index.js + 19 modules
var es = __webpack_require__(14494);
// EXTERNAL MODULE: ./node_modules/classnames/index.js
var classnames = __webpack_require__(94184);
var classnames_default = /*#__PURE__*/__webpack_require__.n(classnames);
// EXTERNAL MODULE: ./packages/mattermost-redux/src/selectors/entities/threads.ts
var threads = __webpack_require__(53672);
// EXTERNAL MODULE: ./packages/mattermost-redux/src/actions/threads.ts
var actions_threads = __webpack_require__(73032);
// EXTERNAL MODULE: ./packages/mattermost-redux/src/actions/channels.ts
var channels = __webpack_require__(88229);
// EXTERNAL MODULE: ./packages/mattermost-redux/src/selectors/entities/posts.ts
var posts = __webpack_require__(81618);
// EXTERNAL MODULE: ./packages/reselect/src/index.js
var src = __webpack_require__(39346);
// EXTERNAL MODULE: ./packages/mattermost-redux/src/selectors/entities/teams.ts
var teams = __webpack_require__(5596);
// EXTERNAL MODULE: ./packages/mattermost-redux/src/selectors/entities/users.ts
var users = __webpack_require__(12707);
// EXTERNAL MODULE: ./selectors/storage.ts
var storage = __webpack_require__(34907);
// EXTERNAL MODULE: ./actions/storage.js
var actions_storage = __webpack_require__(99206);
;// CONCATENATED MODULE: ./stores/hooks.ts
// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.







const currentUserAndTeamSuffix = (0,src/* createSelector */.P1)('currentUserAndTeamSuffix', [users/* getCurrentUserId */.rc, teams/* getCurrentTeamId */.ev], (userId, teamId) => {
  return ":".concat(userId, ":").concat(teamId);
});
const currentUserSuffix = (0,src/* createSelector */.P1)('currentUserSuffix', [users/* getCurrentUserId */.rc], userId => {
  return ":".concat(userId);
});
/**
 *
 * @param initialValue
 * @param name name of stored value, prepended to suffix
 * @param suffix to provide scope; defaults to user and team
 */

function useGlobalState(initialValue, name) {
  const dispatch = (0,es.useDispatch)();
  const suffix = (0,es.useSelector)(currentUserAndTeamSuffix);
  const storedKey = "".concat(name).concat(suffix);
  const value = (0,es.useSelector)((0,storage/* makeGetGlobalItem */.bd)(storedKey, initialValue), es.shallowEqual);
  const setValue = (0,react.useCallback)(newValue => dispatch((0,actions_storage/* setGlobalItem */.Bc)(storedKey, newValue)), [storedKey]);
  return [value, setValue];
}
// EXTERNAL MODULE: ./stores/local_storage_store.jsx
var local_storage_store = __webpack_require__(24493);
// EXTERNAL MODULE: ./actions/global_actions.tsx + 1 modules
var global_actions = __webpack_require__(34288);
// EXTERNAL MODULE: ./actions/views/threads.ts
var views_threads = __webpack_require__(32793);
// EXTERNAL MODULE: ./actions/views/rhs.ts
var rhs = __webpack_require__(53497);
// EXTERNAL MODULE: ./actions/user_actions.jsx
var user_actions = __webpack_require__(47715);
// EXTERNAL MODULE: ./selectors/views/threads.ts
var selectors_views_threads = __webpack_require__(57157);
// EXTERNAL MODULE: ./utils/constants.tsx + 19 modules
var constants = __webpack_require__(66788);
// EXTERNAL MODULE: ./node_modules/prop-types/index.js
var prop_types = __webpack_require__(45697);
var prop_types_default = /*#__PURE__*/__webpack_require__.n(prop_types);
// EXTERNAL MODULE: ./node_modules/core-js/modules/es.object.assign.js
var es_object_assign = __webpack_require__(19601);
;// CONCATENATED MODULE: ./components/widgets/header/header.tsx

const _excluded = ["level", "heading", "subtitle", "right"];


function _objectWithoutProperties(source, excluded) { if (source == null) return {}; var target = _objectWithoutPropertiesLoose(source, excluded); var key, i; if (Object.getOwnPropertySymbols) { var sourceSymbolKeys = Object.getOwnPropertySymbols(source); for (i = 0; i < sourceSymbolKeys.length; i++) { key = sourceSymbolKeys[i]; if (excluded.indexOf(key) >= 0) continue; if (!Object.prototype.propertyIsEnumerable.call(source, key)) continue; target[key] = source[key]; } } return target; }

function _objectWithoutPropertiesLoose(source, excluded) { if (source == null) return {}; var target = {}; var sourceKeys = Object.keys(source); var key, i; for (i = 0; i < sourceKeys.length; i++) { key = sourceKeys[i]; if (excluded.indexOf(key) >= 0) continue; target[key] = source[key]; } return target; }

// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.



const Headings = [react.Fragment, 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'];

const Header = _ref => {
  let {
    level = 0,
    heading,
    subtitle,
    right
  } = _ref,
      attrs = _objectWithoutProperties(_ref, _excluded);

  const H = Headings[level];
  return /*#__PURE__*/react.createElement("header", Object.assign({}, attrs, {
    className: classnames_default()('Header', attrs.className)
  }), /*#__PURE__*/react.createElement("div", {
    className: "left"
  }, /*#__PURE__*/react.createElement(H, null, heading), subtitle ? /*#__PURE__*/react.createElement("p", null, subtitle) : null), /*#__PURE__*/react.createElement("div", {
    className: "spacer"
  }), right);
};

Header.propTypes = {
  heading: (prop_types_default()).node.isRequired,
  level: prop_types_default().oneOf([0, 1, 2, 3, 4, 5, 6]),
  subtitle: (prop_types_default()).node,
  right: (prop_types_default()).node
};
/* harmony default export */ const header = (Header);
;// CONCATENATED MODULE: ./components/widgets/header/index.ts
// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.

// EXTERNAL MODULE: ./components/loading_screen.tsx
var loading_screen = __webpack_require__(19476);
// EXTERNAL MODULE: ./components/no_results_indicator/index.ts
var no_results_indicator = __webpack_require__(86567);
// EXTERNAL MODULE: ./components/threading/hooks.ts
var hooks = __webpack_require__(82704);
;// CONCATENATED MODULE: ./components/threading/common/chat_illustration.tsx
// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.

const ChatIllustration = /*#__PURE__*/react.createElement("svg", {
  width: "228",
  height: "201",
  viewBox: "0 0 228 201",
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg",
  xmlnsXlink: "http://www.w3.org/1999/xlink"
}, /*#__PURE__*/react.createElement("path", {
  d: "M76.5 199.378C111.018 199.378 139 171.396 139 136.878C139 102.361 111.018 74.3785 76.5 74.3785C41.9822 74.3785 14 102.361 14 136.878C14 171.396 41.9822 199.378 76.5 199.378Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M76.5 199.378C111.018 199.378 139 171.396 139 136.878C139 102.361 111.018 74.3785 76.5 74.3785C41.9822 74.3785 14 102.361 14 136.878C14 171.396 41.9822 199.378 76.5 199.378Z",
  fill: "var(--center-channel-color)",
  fillOpacity: "0.16"
}), /*#__PURE__*/react.createElement("path", {
  d: "M89.6523 103.062C91.1459 102.792 92.6812 102.82 94.1631 103.145C95.6169 103.562 96.9528 104.751 97 106.213C96.9429 107.201 96.5757 108.15 95.947 108.932C94.0916 111.638 91.7369 113.992 89 115.878C89 115.878 89.3772 105.599 89.6523 103.062Z",
  fill: "#FFBC1F"
}), /*#__PURE__*/react.createElement("path", {
  d: "M61.8881 100.9C60.5985 100.621 59.2878 100.455 57.9702 100.402C57.2069 100.324 56.4364 100.443 55.7313 100.749C54.5522 101.361 54.0373 102.811 54 104.147C54.018 105.47 54.3127 106.773 54.8645 107.972C55.4163 109.171 56.2128 110.238 57.2015 111.103C59.1924 112.813 61.5051 114.097 64 114.878C63.1567 111.239 61.8881 100.9 61.8881 100.9Z",
  fill: "#FFBC1F"
}), /*#__PURE__*/react.createElement("mask", {
  id: "mask0",
  "mask-type": "alpha",
  maskUnits: "userSpaceOnUse",
  x: "14",
  y: "74",
  width: "125",
  height: "126"
}, /*#__PURE__*/react.createElement("path", {
  d: "M76.5 199.378C111.018 199.378 139 171.396 139 136.878C139 102.361 111.018 74.3784 76.5 74.3784C41.9822 74.3784 14 102.361 14 136.878C14 171.396 41.9822 199.378 76.5 199.378Z",
  fill: "var(--center-channel-bg)"
})), /*#__PURE__*/react.createElement("g", {
  mask: "url(#mask0)"
}, /*#__PURE__*/react.createElement("path", {
  d: "M141 180.509C133.276 174.256 123.746 170.784 114.869 166.304C107.487 162.577 100.497 158.12 94.0041 153C90.476 150.21 86.9555 147.034 85.4987 142.781C83.7536 137.687 85.2863 132.123 86.7582 126.946C88.2757 121.574 89.7249 116.181 91.1057 110.769C92.6232 104.826 93.6399 97.2073 92.995 92.712C92.6763 90.4379 91.0147 86.5946 89.5579 84.3052C87.1427 80.639 83.4063 78.0435 79.1254 77.0583C76.8757 76.6614 74.5591 76.9247 72.4561 77.8163C69.5013 78.9728 66.8499 80.7882 64.704 83.124C62.5581 85.4598 60.9744 88.2543 60.0736 91.2945C56.6593 103.15 64.277 115.476 63.6927 127.802C63.493 131.435 62.6531 135.005 61.2117 138.347C60.2283 140.792 58.8293 143.048 57.0766 145.017C54.459 147.837 51.0067 149.664 47.5697 151.446L21.0747 165.265C18.0398 166.842 15.0049 168.419 12.1748 170.268C9.67857 178.607 34.178 208.33 68.9963 210.073C114.733 212.385 131.986 201.462 141 180.509Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M141 180.509C133.276 174.256 123.746 170.784 114.869 166.304C107.487 162.577 100.497 158.12 94.0041 153C90.476 150.21 86.9555 147.034 85.4987 142.781C83.7536 137.687 85.2863 132.123 86.7582 126.946C88.2757 121.574 89.7249 116.181 91.1057 110.769C92.6232 104.826 93.6399 97.2073 92.995 92.712C92.6763 90.4379 91.0147 86.5946 89.5579 84.3052C87.1427 80.639 83.4063 78.0435 79.1254 77.0583C76.8757 76.6614 74.5591 76.9247 72.4561 77.8163C69.5013 78.9728 66.8499 80.7882 64.704 83.124C62.5581 85.4598 60.9744 88.2543 60.0736 91.2945C56.6593 103.15 64.277 115.476 63.6927 127.802C63.493 131.435 62.6531 135.005 61.2117 138.347C60.2283 140.792 58.8293 143.048 57.0766 145.017C54.459 147.837 51.0067 149.664 47.5697 151.446L21.0747 165.265C18.0398 166.842 15.0049 168.419 12.1748 170.268C9.67857 178.607 34.178 208.33 68.9963 210.073C114.733 212.385 131.986 201.462 141 180.509Z",
  fill: "var(--center-channel-color)",
  fillOpacity: "0.04"
}), /*#__PURE__*/react.createElement("rect", {
  x: "13.5",
  y: "170.878",
  width: "121.5",
  height: "29.5",
  fill: "url(#pattern0)"
})), /*#__PURE__*/react.createElement("path", {
  d: "M72.3684 169.667C94.0078 169.538 99.9495 161.228 102.107 158.84C99.3264 156.995 96.6215 155.039 93.9926 152.971C90.4595 150.181 86.9339 147.004 85.4751 142.75C83.7276 137.655 85.2624 132.09 86.7364 126.911C88.256 121.538 89.7072 116.144 91.0901 110.73C92.6097 104.786 93.6279 97.1658 92.982 92.6696C92.6629 90.3949 90.9989 86.5507 89.5401 84.2609C87.1214 80.5937 83.3798 77.9977 79.0927 77.0122C76.8398 76.6152 74.5199 76.8786 72.414 77.7705C69.4544 78.9263 66.7987 80.7417 64.6496 83.0782C62.5005 85.4146 60.9149 88.2103 60.0139 91.2517C56.5872 103.11 64.2233 115.439 63.6382 127.768C63.4382 131.402 62.5971 134.972 61.1536 138.315C60.1688 140.76 58.7679 143.018 57.0127 144.987C54.3913 147.808 50.9342 149.635 47.4923 151.417L43.6932 153.388C44.9621 155.284 52.2487 169.789 72.3684 169.667Z",
  fill: "#FFBC1F"
}), /*#__PURE__*/react.createElement("mask", {
  id: "mask1",
  "mask-type": "alpha",
  maskUnits: "userSpaceOnUse",
  x: "43",
  y: "76",
  width: "60",
  height: "94"
}, /*#__PURE__*/react.createElement("path", {
  d: "M72.3684 169.667C94.0078 169.538 99.9495 161.228 102.107 158.84C99.3264 156.995 96.6215 155.039 93.9926 152.971C90.4595 150.181 86.934 147.004 85.4751 142.75C83.7276 137.655 85.2624 132.09 86.7364 126.911C88.256 121.538 89.7073 116.144 91.0901 110.73C92.6097 104.786 93.6279 97.1658 92.982 92.6696C92.6629 90.3949 90.9989 86.5507 89.5401 84.2609C87.1214 80.5937 83.3798 77.9977 79.0927 77.0122C76.8399 76.6152 74.5199 76.8786 72.414 77.7705C69.4544 78.9263 66.7987 80.7417 64.6496 83.0782C62.5005 85.4146 60.9149 88.2103 60.0139 91.2517C56.5872 103.11 64.2233 115.439 63.6382 127.768C63.4382 131.402 62.5971 134.972 61.1536 138.315C60.1688 140.76 58.7679 143.018 57.0127 144.987C54.3913 147.808 50.9342 149.635 47.4923 151.417L43.6932 153.388C44.9621 155.284 52.2487 169.789 72.3684 169.667Z",
  fill: "#FFBC1F"
})), /*#__PURE__*/react.createElement("g", {
  mask: "url(#mask1)"
}, /*#__PURE__*/react.createElement("rect", {
  x: "43.5",
  y: "76.3785",
  width: "59",
  height: "93.5",
  fill: "url(#pattern1)"
})), /*#__PURE__*/react.createElement("path", {
  d: "M84.9205 138.14C84.8876 137.284 84.9488 136.428 85.1028 135.585C85.1061 135.565 85.1061 135.545 85.1028 135.524C85.1001 133.503 85.5328 131.504 86.3717 129.663L86.2729 129.762C83.3029 132.637 79.3404 134.266 75.2025 134.311C73.5359 134.313 71.8803 134.041 70.3017 133.508C70.2068 133.475 70.1029 133.479 70.0112 133.52C69.9195 133.561 69.8468 133.635 69.8079 133.727V133.727C69.7785 133.797 69.77 133.873 69.7835 133.947C69.797 134.021 69.8319 134.09 69.8839 134.144C76.6386 140.301 84.7989 138.815 84.9433 138.633C84.9205 138.497 84.9205 138.315 84.9205 138.14Z",
  fill: "#CC8F00"
}), /*#__PURE__*/react.createElement("path", {
  d: "M74.3606 118.774C74.9163 119.24 75.5645 119.583 76.2626 119.78C76.9606 119.978 77.6927 120.025 78.4105 119.92C79.1284 119.815 79.8158 119.559 80.4273 119.169C81.0389 118.779 81.5608 118.265 81.9587 117.66C82.3158 117.098 81.4268 116.59 81.0773 117.144C80.7606 117.617 80.3487 118.018 79.8679 118.323C79.387 118.628 78.8478 118.83 78.2846 118.915C77.7213 119.001 77.1464 118.969 76.5964 118.82C76.0464 118.672 75.5334 118.411 75.09 118.054C74.5886 117.629 73.8591 118.349 74.3302 118.774H74.3606Z",
  fill: "#66320A"
}), /*#__PURE__*/react.createElement("path", {
  d: "M64.901 99.3407C66.9981 98.8934 70.3488 98.355 72.4307 99.2346C74.5126 100.114 74.0795 101.449 74.0035 103.094C73.9992 103.771 73.8543 104.44 73.578 105.058C73.1297 106.104 72.7118 106.233 71.6633 106.521C69.5358 107.12 66.0711 107.408 64.3539 105.801C63.0775 104.61 62.5684 99.7881 64.8554 99.3786C65.5012 99.2649 65.2277 98.2792 64.5819 98.393C64.0758 98.4575 63.6028 98.6787 63.2293 99.0255C62.8558 99.3724 62.6008 99.8274 62.5 100.326C62.1077 102.207 62.2687 104.161 62.9635 105.952C63.5941 107.257 65.2429 107.567 66.565 107.787C68.4492 108.102 70.3809 107.98 72.2104 107.431C73.6084 107.006 74.6721 106.385 74.9229 104.861C75.1508 103.458 75.5307 100.986 74.8469 99.6744C74.1631 98.3626 72.7194 98.2034 71.5037 98.0366C69.2103 97.7476 66.8842 97.8553 64.6275 98.355C63.9816 98.4915 64.2552 99.4772 64.901 99.3407V99.3407Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M89.382 99.7047C87.1863 98.935 84.8706 98.5627 82.5437 98.6053C81.3205 98.6053 79.7932 98.7342 79.0562 99.8867C78.7236 100.598 78.5647 101.377 78.5927 102.161C78.4209 103.239 78.3699 104.333 78.4408 105.422C78.6611 106.84 79.9072 107.461 81.1229 107.954C83.4403 108.894 88.9793 110.009 90.3394 107.067C91.3195 104.959 92.3833 100.63 89.382 99.7199C89.2511 99.6837 89.111 99.7009 88.9928 99.7677C88.8745 99.8345 88.7877 99.9455 88.7514 100.076C88.7151 100.207 88.7324 100.347 88.7993 100.465C88.8663 100.583 88.9775 100.669 89.1085 100.706C91.3044 101.373 90.0355 106.514 88.5538 107.476C87.6876 108.038 85.8641 107.878 84.8612 107.803C83.6141 107.718 82.3907 107.42 81.2445 106.923C79.6337 106.21 79.4665 104.861 79.5805 103.291C79.6945 101.722 79.7552 100 81.5788 99.7199C84.134 99.4495 86.717 99.7873 89.1161 100.706C89.7391 100.918 90.0127 99.9473 89.3896 99.7274L89.382 99.7047Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M74.8469 101.039C76.1119 100.68 77.4654 100.802 78.6459 101.38C79.231 101.676 79.7552 100.789 79.1626 100.501C77.7398 99.785 76.1003 99.6254 74.5658 100.053C74.4348 100.089 74.3232 100.174 74.2555 100.292C74.1878 100.409 74.1696 100.548 74.2048 100.679C74.2401 100.81 74.326 100.921 74.4435 100.989C74.5611 101.056 74.7007 101.074 74.8317 101.039H74.8469Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M93.4622 102.04C92.5832 102.065 91.7337 102.362 91.0308 102.889C90.5066 103.276 91.0308 104.163 91.5475 103.769C92.0977 103.343 92.7664 103.096 93.4622 103.064C94.1156 103.064 94.1232 102.017 93.4622 102.04V102.04Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M59.1492 100.857C60.1784 100.527 61.2937 100.589 62.2796 101.032C62.8723 101.305 63.3966 100.425 62.7963 100.152C61.5696 99.5763 60.1722 99.4763 58.8757 99.8715C58.8108 99.8894 58.7501 99.9199 58.6971 99.9612C58.6441 100.003 58.5997 100.054 58.5665 100.112C58.5334 100.171 58.5121 100.235 58.5038 100.302C58.4956 100.368 58.5006 100.436 58.5186 100.501C58.5365 100.566 58.5671 100.626 58.6085 100.679C58.6499 100.732 58.7014 100.776 58.7599 100.809C58.8185 100.842 58.883 100.864 58.9498 100.872C59.0166 100.88 59.0844 100.875 59.1492 100.857V100.857Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M67.9402 103.61C68.7417 103.248 69.6279 103.114 70.5008 103.223C71.1542 103.299 71.1542 102.275 70.5008 102.207C69.4463 102.093 68.3809 102.274 67.4235 102.73C66.8233 103.011 67.34 103.89 67.9402 103.61V103.61Z",
  fill: "#66320A"
}), /*#__PURE__*/react.createElement("path", {
  d: "M83.6835 104.193C84.0466 104.158 84.413 104.199 84.7596 104.312C85.1061 104.426 85.4253 104.61 85.697 104.853C86.1832 105.293 86.9127 104.572 86.4188 104.133C86.0536 103.793 85.6223 103.533 85.1516 103.367C84.681 103.201 84.1812 103.134 83.6835 103.17C83.03 103.215 83.0224 104.239 83.6835 104.193V104.193Z",
  fill: "#66320A"
}), /*#__PURE__*/react.createElement("path", {
  d: "M75.7815 101.191C75.6143 104.11 75.2724 107.014 75.0217 109.933C74.8697 111.753 74.3227 114.968 77.4379 113.724C78.0381 113.482 77.7798 112.488 77.1643 112.739C75.5763 113.383 75.8574 112.428 75.9334 111.313C76.0094 110.199 76.1234 108.963 76.2298 107.795C76.4197 105.596 76.6628 103.405 76.792 101.206C76.83 100.546 75.8043 100.546 75.7663 101.206L75.7815 101.191Z",
  fill: "#66320A"
}), /*#__PURE__*/react.createElement("path", {
  d: "M95.9605 81.0128C95.8076 79.6869 95.3539 78.4132 94.6337 77.2885C93.9135 76.1639 92.9459 75.2179 91.8044 74.5224C88.6588 72.6496 84.7154 72.9377 81.0683 73.2941C81.3418 71.7776 79.5866 70.5721 78.029 70.4128C75.4381 70.155 72.9535 71.3909 70.6893 72.6875C67.917 74.1405 65.3976 76.03 63.228 78.2832C63.1423 78.3017 63.0637 78.3444 63.0017 78.4062C62.9397 78.4681 62.8969 78.5465 62.8784 78.632C62.875 78.6521 62.875 78.6726 62.8784 78.6926C61.8807 79.8823 61.0897 81.2304 60.5382 82.6809C60.2916 83.1174 60.1099 83.5874 59.9988 84.076C59.5914 85.6228 59.5783 87.2467 59.9608 88.7998C59.9642 89.0773 60.0635 89.3451 60.2419 89.558L60.3331 89.7476C60.2156 89.7368 60.0982 89.7684 60.0021 89.8365C59.9059 89.9047 59.8374 90.0049 59.8088 90.1191C59.7176 90.6423 59.6341 91.173 59.5429 91.6962C59.512 91.8277 59.5326 91.966 59.6003 92.083C59.6681 92.1999 59.778 92.2866 59.9076 92.3255C60.0376 92.3604 60.1761 92.3424 60.2928 92.2757C60.4096 92.2089 60.4951 92.0987 60.5306 91.9692C60.6218 91.4384 60.7054 90.9152 60.7966 90.3845C60.8176 90.2581 60.799 90.1284 60.7434 90.0129C60.7986 90.0238 60.8554 90.0238 60.9105 90.0129H60.9561C61.0647 90.0736 61.1917 90.0925 61.3132 90.066C63.9569 89.461 66.3073 87.9572 67.9616 85.8124C70.7957 85.2968 74.7011 85.6986 78.7661 86.1232C79.1896 86.6397 79.7047 87.0741 80.2857 87.4046C81.2329 87.858 82.2748 88.0789 83.3249 88.0491C84.9232 88.1033 86.5041 87.7036 87.8838 86.8966C87.9229 87.5975 88.2226 88.2587 88.7242 88.7508C89.2259 89.243 89.8933 89.5308 90.5963 89.558C90.974 89.5537 91.3476 89.4791 91.698 89.3381C92.1173 90.7068 92.3298 92.1302 92.3286 93.5614C92.3439 93.6864 92.4045 93.8016 92.499 93.885C92.5935 93.9685 92.7153 94.0146 92.8415 94.0146C92.9677 94.0146 93.0895 93.9685 93.184 93.885C93.2785 93.8016 93.3391 93.6864 93.3544 93.5614C93.3428 91.9575 93.0866 90.3647 92.5946 88.8377C92.8408 88.6649 93.0744 88.4749 93.2936 88.269C94.2643 87.3434 95.0095 86.208 95.4718 84.9501C95.9341 83.6922 96.1013 82.3453 95.9605 81.0128V81.0128Z",
  fill: "#66320A"
}), /*#__PURE__*/react.createElement("path", {
  d: "M59.9228 93.6145C59.8733 93.564 59.8142 93.524 59.7489 93.4966C59.6837 93.4693 59.6136 93.4552 59.5429 93.4552C59.4721 93.4552 59.4021 93.4693 59.3368 93.4966C59.2716 93.524 59.2125 93.564 59.163 93.6145C59.1115 93.6633 59.0705 93.7221 59.0425 93.7873C59.0145 93.8525 59 93.9227 59 93.9936C59 94.0645 59.0145 94.1347 59.0425 94.1998C59.0705 94.265 59.1115 94.3238 59.163 94.3727L59.277 94.4864C59.3752 94.577 59.5041 94.6273 59.6379 94.6273C59.7716 94.6273 59.9005 94.577 59.9988 94.4864C60.0503 94.4376 60.0913 94.3787 60.1193 94.3136C60.1473 94.2484 60.1618 94.1782 60.1618 94.1073C60.1618 94.0364 60.1473 93.9662 60.1193 93.901C60.0913 93.8359 60.0503 93.7771 59.9988 93.7282L59.9228 93.6145Z",
  fill: "#66320A"
}), /*#__PURE__*/react.createElement("path", {
  d: "M92.9897 94.3727C92.8546 94.3766 92.7262 94.4324 92.6314 94.5285C92.5365 94.6246 92.4825 94.7535 92.4806 94.8883V95.6466C92.4806 95.7813 92.5342 95.9105 92.6297 96.0058C92.7252 96.101 92.8547 96.1546 92.9897 96.1546C93.1247 96.1546 93.2542 96.101 93.3497 96.0058C93.4451 95.9105 93.4988 95.7813 93.4988 95.6466V94.8883C93.4969 94.7535 93.4429 94.6246 93.348 94.5285C93.2531 94.4324 93.1248 94.3766 92.9897 94.3727Z",
  fill: "#66320A"
}), /*#__PURE__*/react.createElement("mask", {
  id: "mask2",
  "mask-type": "alpha",
  maskUnits: "userSpaceOnUse",
  x: "59",
  y: "70",
  width: "38",
  height: "25"
}, /*#__PURE__*/react.createElement("path", {
  d: "M95.9606 81.0128C95.8077 79.6869 95.3539 78.4132 94.6337 77.2885C93.9136 76.1639 92.9459 75.2179 91.8044 74.5224C88.6588 72.6496 84.7154 72.9377 81.0683 73.2941C81.3418 71.7776 79.5867 70.5721 78.0291 70.4128C75.4381 70.155 72.9535 71.3909 70.6893 72.6875C67.9171 74.1405 65.3976 76.03 63.228 78.2832C63.1423 78.3017 63.0637 78.3444 63.0018 78.4062C62.9398 78.4681 62.897 78.5465 62.8785 78.632C62.8751 78.6521 62.8751 78.6726 62.8785 78.6926C61.8807 79.8823 61.0897 81.2304 60.5383 82.6809C60.2917 83.1174 60.1099 83.5874 59.9988 84.076C59.5914 85.6228 59.5783 87.2467 59.9608 88.7998C59.9643 89.0773 60.0636 89.3451 60.2419 89.558L60.3331 89.7476C60.2157 89.7368 60.0982 89.7684 60.0021 89.8365C59.9059 89.9047 59.8374 90.0049 59.8089 90.1191C59.7177 90.6423 59.6341 91.173 59.5429 91.6962C59.512 91.8277 59.5326 91.966 59.6004 92.083C59.6681 92.1999 59.778 92.2866 59.9076 92.3255C60.0376 92.3604 60.1761 92.3424 60.2928 92.2757C60.4096 92.2089 60.4951 92.0987 60.5307 91.9692C60.6218 91.4384 60.7054 90.9152 60.7966 90.3845C60.8176 90.2581 60.7991 90.1284 60.7434 90.0129C60.7986 90.0238 60.8554 90.0238 60.9106 90.0129H60.9562C61.0647 90.0736 61.1917 90.0925 61.3133 90.066C63.9569 89.461 66.3073 87.9572 67.9616 85.8124C70.7957 85.2968 74.7011 85.6986 78.7661 86.1232C79.1896 86.6397 79.7047 87.0741 80.2857 87.4046C81.2329 87.858 82.2749 88.0789 83.3249 88.0491C84.9232 88.1033 86.5041 87.7036 87.8838 86.8966C87.923 87.5975 88.2227 88.2587 88.7243 88.7508C89.2259 89.243 89.8934 89.5308 90.5963 89.558C90.9741 89.5537 91.3477 89.4791 91.698 89.3381C92.1173 90.7068 92.3298 92.1302 92.3287 93.5614C92.3439 93.6864 92.4045 93.8016 92.499 93.885C92.5935 93.9685 92.7153 94.0146 92.8415 94.0146C92.9677 94.0146 93.0896 93.9685 93.1841 93.885C93.2785 93.8016 93.3391 93.6864 93.3544 93.5614C93.3429 91.9575 93.0867 90.3647 92.5946 88.8377C92.8408 88.6649 93.0744 88.4749 93.2936 88.269C94.2644 87.3434 95.0095 86.208 95.4718 84.9501C95.9342 83.6922 96.1013 82.3453 95.9606 81.0128V81.0128Z",
  fill: "#66320A"
})), /*#__PURE__*/react.createElement("g", {
  mask: "url(#mask2)"
}, /*#__PURE__*/react.createElement("rect", {
  x: "59.5",
  y: "70.3785",
  width: "37",
  height: "23.5",
  fill: "url(#pattern2)"
})), /*#__PURE__*/react.createElement("path", {
  d: "M43.5 153.378C50.6949 164.128 58.5583 171.905 80.5 169.41",
  stroke: "#66320A",
  strokeWidth: "0.91",
  strokeMiterlimit: "10",
  strokeLinecap: "round"
}), /*#__PURE__*/react.createElement("path", {
  d: "M128.698 24.8786H215.802C217.398 24.8726 218.979 25.1799 220.456 25.7831C221.933 26.3862 223.276 27.2733 224.408 28.3937C225.541 29.5141 226.441 30.8459 227.058 32.313C227.674 33.7801 227.994 35.3538 228 36.9443V92.1179C227.995 93.709 227.676 95.2835 227.06 96.7515C226.444 98.2195 225.544 99.5522 224.411 100.673C223.278 101.795 221.935 102.682 220.458 103.286C218.98 103.89 217.398 104.197 215.802 104.191H202.95V124.878L183.643 104.184H128.698C127.102 104.19 125.521 103.882 124.044 103.279C122.567 102.676 121.224 101.789 120.092 100.669C118.959 99.5481 118.059 98.2163 117.442 96.7492C116.826 95.2821 116.506 93.7084 116.5 92.1179V36.9443C116.506 35.3538 116.826 33.7801 117.442 32.313C118.059 30.8459 118.959 29.5141 120.092 28.3937C121.224 27.2733 122.567 26.3862 124.044 25.7831C125.521 25.1799 127.102 24.8726 128.698 24.8786V24.8786Z",
  fill: "var(--button-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M161.5 104.378H215.919C217.499 104.384 219.066 104.077 220.528 103.475C221.991 102.873 223.321 101.987 224.443 100.868C225.565 99.749 226.456 98.419 227.067 96.9539C227.677 95.4887 227.994 93.9171 228 92.3287V58.8785C228 58.8785 224.201 89.8115 223.524 92.5328C222.847 95.2542 221.493 99.3363 215.069 100.009C208.644 100.682 161.5 104.378 161.5 104.378Z",
  fill: "black",
  fillOpacity: "0.32"
}), /*#__PURE__*/react.createElement("path", {
  d: "M141.5 56.8785C143.082 56.8785 144.629 57.3477 145.945 58.2267C147.26 59.1058 148.286 60.3552 148.891 61.817C149.497 63.2788 149.655 64.8874 149.346 66.4392C149.038 67.9911 148.276 69.4165 147.157 70.5353C146.038 71.6541 144.613 72.4161 143.061 72.7248C141.509 73.0334 139.9 72.875 138.439 72.2695C136.977 71.664 135.727 70.6386 134.848 69.323C133.969 68.0075 133.5 66.4607 133.5 64.8785C133.502 62.7574 134.346 60.7237 135.845 59.2239C137.345 57.724 139.379 56.8805 141.5 56.8785Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M171.508 56.8785C173.09 56.88 174.636 57.3505 175.95 58.2305C177.265 59.1105 178.289 60.3604 178.893 61.8224C179.498 63.2843 179.655 64.8926 179.345 66.444C179.036 67.9953 178.273 69.42 177.154 70.5381C176.035 71.6561 174.61 72.4173 173.058 72.7253C171.506 73.0334 169.898 72.8745 168.437 72.2688C166.975 71.663 165.726 70.6377 164.848 69.3222C163.969 68.0068 163.5 66.4604 163.5 64.8785C163.5 63.8273 163.707 62.7863 164.11 61.8152C164.512 60.8441 165.102 59.9619 165.846 59.2189C166.59 58.4759 167.472 57.8868 168.444 57.4852C169.415 57.0836 170.457 56.8775 171.508 56.8785V56.8785Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M201.5 56.8785C203.082 56.8785 204.629 57.3477 205.945 58.2267C207.26 59.1058 208.286 60.3552 208.891 61.817C209.497 63.2788 209.655 64.8874 209.346 66.4392C209.038 67.9911 208.276 69.4165 207.157 70.5353C206.038 71.6541 204.613 72.4161 203.061 72.7248C201.509 73.0334 199.9 72.875 198.439 72.2695C196.977 71.664 195.727 70.6386 194.848 69.323C193.969 68.0075 193.5 66.4607 193.5 64.8785C193.5 62.7567 194.343 60.7219 195.843 59.2216C197.343 57.7213 199.378 56.8785 201.5 56.8785Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M125.898 47.7743C126.814 44.666 128.358 41.7626 130.441 39.2305C132.525 36.6984 135.107 34.5875 138.039 33.0188C138.207 32.9305 138.341 32.7917 138.421 32.6236C138.5 32.4555 138.521 32.2671 138.479 32.0868C138.438 31.9065 138.336 31.7441 138.191 31.624C138.045 31.5039 137.863 31.4328 137.671 31.4212C132.016 31.0972 120.594 32.2577 124.185 47.6914C124.223 47.8778 124.325 48.0466 124.474 48.1709C124.623 48.2952 124.811 48.3679 125.008 48.3774C125.205 48.3869 125.4 48.3327 125.561 48.2235C125.722 48.1142 125.841 47.9561 125.898 47.7743V47.7743Z",
  fill: "white",
  fillOpacity: "0.24"
}), /*#__PURE__*/react.createElement("path", {
  d: "M73.4957 0.378531H9.02713C7.84566 0.374539 6.67498 0.602539 5.58192 1.04951C4.48886 1.49648 3.49483 2.15366 2.65658 2.98354C1.81833 3.81343 1.15228 4.79975 0.696467 5.8862C0.24065 6.97265 0.00399142 8.13796 0 9.31558V50.1544C0.00399142 51.332 0.24065 52.4973 0.696467 53.5838C1.15228 54.6702 1.81833 55.6565 2.65658 56.4864C3.49483 57.3163 4.48886 57.9735 5.58192 58.4204C6.67498 58.8674 7.84566 59.0954 9.02713 59.0914H18.5414V74.3785L32.8128 59.0914H73.4729C74.6543 59.0954 75.825 58.8674 76.9181 58.4204C78.0112 57.9735 79.0052 57.3163 79.8434 56.4864C80.6817 55.6565 81.3477 54.6702 81.8035 53.5838C82.2593 52.4973 82.496 51.332 82.5 50.1544V9.31558C82.4919 6.94123 81.5395 4.66697 79.8515 2.9916C78.1635 1.31623 75.8778 0.376508 73.4957 0.378531V0.378531Z",
  fill: "#FFBC1F"
}), /*#__PURE__*/react.createElement("path", {
  d: "M32.5 58.8784H73.4161C74.605 58.8823 75.783 58.6593 76.883 58.222C77.9829 57.7847 78.9832 57.1417 79.8267 56.3298C80.6702 55.5179 81.3405 54.5529 81.7991 53.49C82.2578 52.427 82.496 51.287 82.5 50.1349V25.8785C82.5 25.8785 79.6431 48.3089 79.1299 50.2833C78.6167 52.2577 77.598 55.2118 72.7727 55.7016C67.9473 56.1915 32.5 58.8784 32.5 58.8784Z",
  fill: "black",
  fillOpacity: "0.32"
}), /*#__PURE__*/react.createElement("path", {
  d: "M19 23.8785C20.1867 23.8785 21.3467 24.2304 22.3334 24.8897C23.3201 25.5489 24.0892 26.486 24.5433 27.5824C24.9974 28.6787 25.1162 29.8851 24.8847 31.049C24.6532 32.2129 24.0818 33.282 23.2426 34.1211C22.4035 34.9602 21.3344 35.5317 20.1705 35.7632C19.0067 35.9947 17.8003 35.8759 16.7039 35.4218C15.6075 34.9676 14.6705 34.1986 14.0112 33.2119C13.3519 32.2252 13 31.0652 13 29.8785C12.999 29.0903 13.1535 28.3096 13.4546 27.5811C13.7558 26.8527 14.1977 26.1909 14.7551 25.6335C15.3124 25.0762 15.9743 24.6343 16.7027 24.3331C17.4311 24.0319 18.2118 23.8774 19 23.8785V23.8785Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M41.5 23.8785C42.6867 23.8785 43.8467 24.2304 44.8334 24.8897C45.8201 25.5489 46.5892 26.486 47.0433 27.5824C47.4974 28.6787 47.6162 29.8851 47.3847 31.049C47.1532 32.2129 46.5818 33.282 45.7426 34.1211C44.9035 34.9602 43.8344 35.5317 42.6705 35.7632C41.5067 35.9947 40.3003 35.8759 39.2039 35.4218C38.1075 34.9676 37.1705 34.1986 36.5112 33.2119C35.8519 32.2252 35.5 31.0652 35.5 29.8785C35.499 29.0903 35.6535 28.3096 35.9546 27.5811C36.2558 26.8527 36.6977 26.1909 37.2551 25.6335C37.8124 25.0762 38.4743 24.6343 39.2027 24.3331C39.9311 24.0319 40.7118 23.8774 41.5 23.8785V23.8785Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M63.9921 23.8785C65.1792 23.8769 66.34 24.2275 67.3277 24.8858C68.3154 25.5442 69.0857 26.4807 69.541 27.5769C69.9963 28.6731 70.1163 29.8798 69.8857 31.0442C69.6551 32.2086 69.0842 33.2784 68.2454 34.1183C67.4066 34.9582 66.3375 35.5305 65.1734 35.7626C64.0093 35.9948 62.8025 35.8764 61.7057 35.4225C60.6089 34.9686 59.6714 34.1996 59.0117 33.2127C58.3521 32.2259 58 31.0655 58 29.8785C57.999 29.0909 58.1532 28.3109 58.4539 27.583C58.7545 26.8551 59.1957 26.1936 59.7523 25.6363C60.3088 25.0791 60.9697 24.637 61.6972 24.3354C62.4247 24.0337 63.2046 23.8785 63.9921 23.8785V23.8785Z",
  fill: "var(--center-channel-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M6.7637 16.9281C7.43137 14.6443 8.55997 12.5107 10.0845 10.6501C11.609 8.78959 13.4993 7.23886 15.6466 6.08727C15.7738 6.02514 15.8761 5.92399 15.9374 5.79966C15.9987 5.67533 16.0156 5.53486 15.9854 5.40026C15.9552 5.26565 15.8797 5.14452 15.7707 5.05584C15.6616 4.96717 15.5252 4.91596 15.3828 4.91024C11.2401 4.67033 2.87695 5.525 5.49915 16.8681C5.52681 17.0065 5.60175 17.132 5.71197 17.2244C5.8222 17.3168 5.96133 17.3708 6.10707 17.3777C6.25282 17.3846 6.39675 17.3441 6.51581 17.2625C6.63486 17.181 6.72215 17.0632 6.7637 16.9281V16.9281Z",
  fill: "white",
  fillOpacity: "0.32"
}), /*#__PURE__*/react.createElement("defs", null, /*#__PURE__*/react.createElement("pattern", {
  id: "pattern0",
  patternContentUnits: "objectBoundingBox",
  width: "1",
  height: "1"
}, /*#__PURE__*/react.createElement("use", {
  xlinkHref: "#image0",
  transform: "scale(0.00411523 0.0169492)"
})), /*#__PURE__*/react.createElement("pattern", {
  id: "pattern1",
  patternContentUnits: "objectBoundingBox",
  width: "1",
  height: "1"
}, /*#__PURE__*/react.createElement("use", {
  xlinkHref: "#image1",
  transform: "scale(0.00847458 0.00534759)"
})), /*#__PURE__*/react.createElement("pattern", {
  id: "pattern2",
  patternContentUnits: "objectBoundingBox",
  width: "1",
  height: "1"
}, /*#__PURE__*/react.createElement("use", {
  xlinkHref: "#image2",
  transform: "scale(0.0135135 0.0212766)"
})), /*#__PURE__*/react.createElement("image", {
  id: "image0",
  width: "243",
  height: "59",
  xlinkHref: "data:image/png;base64,"
}), /*#__PURE__*/react.createElement("image", {
  id: "image1",
  width: "118",
  height: "187",
  xlinkHref: "data:image/png;base64,"
}), /*#__PURE__*/react.createElement("image", {
  id: "image2",
  width: "74",
  height: "47",
  xlinkHref: "data:image/png;base64,"
})));
/* harmony default export */ const chat_illustration = (ChatIllustration);
// EXTERNAL MODULE: ./components/threading/thread_viewer/index.ts + 24 modules
var thread_viewer = __webpack_require__(63006);
// EXTERNAL MODULE: ./node_modules/react-intl/lib/src/components/message.js
var message = __webpack_require__(44012);
// EXTERNAL MODULE: ./utils/utils.tsx + 25 modules
var utils = __webpack_require__(15883);
// EXTERNAL MODULE: ./actions/telemetry_actions.jsx
var telemetry_actions = __webpack_require__(59925);
// EXTERNAL MODULE: ./components/widgets/simple_tooltip/index.ts + 1 modules
var simple_tooltip = __webpack_require__(88771);
// EXTERNAL MODULE: ./components/threading/common/button/index.ts + 1 modules
var common_button = __webpack_require__(12874);
;// CONCATENATED MODULE: ./components/threading/common/balloon_illustration.tsx
// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.

const BalloonIllustration = /*#__PURE__*/react.createElement("svg", {
  width: "175",
  height: "207",
  viewBox: "0 0 175 207",
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg"
}, /*#__PURE__*/react.createElement("g", {
  clipPath: "url(#clip0)"
}, /*#__PURE__*/react.createElement("path", {
  d: "M86.3343 124.207C72.3574 144.087 72.8886 156.7 88.105 165.391C109.903 177.797 107.967 199.007 84.6344 206.581",
  stroke: "var(--center-channel-color)",
  strokeOpacity: "0.56",
  strokeWidth: "1.5",
  strokeMiterlimit: "10"
}), /*#__PURE__*/react.createElement("g", {
  opacity: "0.7"
}, /*#__PURE__*/react.createElement("path", {
  opacity: "0.7",
  d: "M138.21 70.6109C149.153 73.169 155.369 84.7898 147.176 96.9187C138.984 109.048 130.514 100.032 134.468 94.9396C138.423 89.847 156.484 102.508 144.355 117C133.465 129.997 134.468 149.493 145.199 152.766",
  stroke: "var(--center-channel-color)",
  strokeOpacity: "0.56",
  strokeWidth: "1.34",
  strokeMiterlimit: "10"
}), /*#__PURE__*/react.createElement("path", {
  opacity: "0.72",
  d: "M142.224 70.6109C142.23 70.82 142.291 71.0239 142.401 71.2017L142.755 72.0938C142.807 72.1836 142.834 72.2854 142.834 72.3892C142.834 72.4929 142.807 72.5947 142.755 72.6846V72.9799C142.262 73.5564 141.572 73.929 140.819 74.0257C140.038 74.1697 139.232 74.047 138.529 73.6771C138.193 73.5052 137.894 73.2682 137.65 72.9799L137.473 72.6905V72.0997L138.181 70.5991C134.893 69.5062 131.352 67.3084 127.616 63.5097C123.806 59.544 121.085 54.6596 119.718 49.3308C119.163 47.3606 118.884 45.3224 118.892 43.2752C119.004 37.2492 120.987 31.2172 124.422 27.7788C129.074 23.1234 134.008 22.1368 140.111 22.0364C146.338 22.1545 151.255 23.8442 155.363 27.7966C158.845 31.1463 161.265 37.2492 161.513 43.2752C161.518 45.3227 161.238 47.3608 160.681 49.3308C159.203 54.6365 156.432 59.4921 152.618 63.4624C148.876 67.3143 145.388 69.5179 142.224 70.6109Z",
  fill: "var(--button-bg)"
}), /*#__PURE__*/react.createElement("path", {
  opacity: "0.7",
  d: "M129.073 41.9104C129.112 43.0661 128.875 44.2143 128.383 45.2602C127.922 46.1641 127.35 46.6131 126.677 46.6131C126.004 46.6131 125.414 46.1641 124.906 45.2602C124.379 44.2248 124.123 43.0721 124.163 41.9104C124.119 40.7674 124.376 39.6329 124.906 38.6198C125.402 37.7632 125.992 37.3318 126.677 37.3318C127.362 37.3318 127.922 37.7632 128.383 38.6198C128.878 39.6438 129.115 40.7736 129.073 41.9104Z",
  fill: "white",
  fillOpacity: "0.32"
})), /*#__PURE__*/react.createElement("g", {
  opacity: "0.3"
}, /*#__PURE__*/react.createElement("path", {
  opacity: "0.3",
  d: "M30.6157 37.9462C41.7063 46.808 39.9887 65.595 24.247 70.7881C1.26311 78.3738 -3.29944 95.991 2.93349 103.281",
  stroke: "var(--center-channel-color)",
  strokeOpacity: "0.56",
  strokeWidth: "1.34",
  strokeMiterlimit: "10"
}), /*#__PURE__*/react.createElement("path", {
  opacity: "0.64",
  d: "M32.9058 37.6863C32.8762 37.8466 32.8905 38.012 32.9472 38.1649C32.9885 38.413 33.0298 38.6552 33.077 38.9033C33.1053 38.9785 33.1114 39.0602 33.0947 39.1387C33.078 39.2173 33.039 39.2894 32.9826 39.3464L32.9354 39.5709C32.4729 39.9334 31.8921 40.1103 31.3063 40.0672C30.6895 40.0583 30.0942 39.8395 29.6182 39.4468C29.3799 39.2649 29.1794 39.0381 29.028 38.7792L28.9394 38.5311C28.9689 38.3834 29.0044 38.2357 29.0339 38.0881C29.2877 37.7395 29.5415 37.3968 29.8012 37.0601C27.4756 35.719 25.1206 33.5154 22.8777 29.9706C20.601 26.3712 19.2917 22.2442 19.0765 17.9895C18.9611 16.4071 19.0664 14.8164 19.3894 13.2632C20.4158 9.03049 22.7248 5.2191 26 2.35135C28.8745 -0.177223 34.1572 -0.443077 38.8141 0.431288C43.5361 1.4888 47.0008 3.53287 49.5093 7.17803C51.6341 10.2619 52.5372 15.2955 51.7758 19.8977C51.4632 21.4544 50.9347 22.9597 50.2057 24.37C48.2581 28.1712 45.3985 31.4292 41.8834 33.8521C38.46 36.2034 35.5088 37.3495 32.9058 37.6863Z",
  fill: "var(--button-bg)"
}), /*#__PURE__*/react.createElement("path", {
  opacity: "0.3",
  d: "M27.3635 13.8539C27.2136 14.7369 26.8555 15.5714 26.3188 16.288C26.168 16.5467 25.9457 16.7563 25.6787 16.8915C25.4116 17.0267 25.1112 17.0818 24.8137 17.0501C24.3002 16.9438 23.9224 16.5125 23.6863 15.7503C23.4459 14.8825 23.4296 13.9677 23.6391 13.0918C23.786 12.2229 24.1585 11.408 24.7192 10.7287C24.8858 10.4813 25.1169 10.2844 25.3875 10.1594C25.6581 10.0344 25.9577 9.9861 26.2539 10.0197C26.7674 10.1261 27.1333 10.5396 27.3517 11.2663C27.5632 12.1154 27.5673 13.003 27.3635 13.8539Z",
  fill: "white",
  fillOpacity: "0.32"
})), /*#__PURE__*/react.createElement("path", {
  d: "M83.6605 122.943C83.6572 123.268 83.5784 123.587 83.4303 123.876L82.964 125.27C82.8885 125.413 82.8491 125.573 82.8491 125.734C82.8491 125.896 82.8885 126.055 82.964 126.198V126.665C83.2501 127.116 83.6236 127.506 84.0626 127.81C84.5016 128.114 84.997 128.328 85.5197 128.437C86.0358 128.556 86.5709 128.564 87.0905 128.463C87.6101 128.361 88.1025 128.151 88.5358 127.846C89.0062 127.538 89.408 127.136 89.7163 126.665L89.9465 126.198V125.27L89.0198 122.943C93.3483 121.088 98.0308 116.952 103.068 110.536C108.004 104.297 111.671 97.1493 113.863 89.4984C114.647 86.8581 115.045 84.1178 115.043 81.3632C114.89 73.3088 112.104 66.5699 106.686 61.1465C101.267 55.7231 94.5327 52.9326 86.4818 52.775C78.2775 52.9326 71.5055 55.7231 66.1658 61.1465C60.8261 66.5699 58.0008 73.3088 57.69 81.3632C57.6912 84.1176 58.0888 86.8574 58.8704 89.4984C61.2112 97.1344 64.95 104.268 69.8961 110.536C74.9092 116.948 79.4973 121.084 83.6605 122.943Z",
  fill: "var(--button-bg)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M63.3681 81.3337C63.291 83.6349 63.8116 85.9168 64.8791 87.9564C65.8884 89.7287 67.1279 90.6267 68.5976 90.6267C70.0673 90.6267 71.3481 89.7405 72.4282 87.9564C73.5697 85.9411 74.1315 83.6489 74.0514 81.3337C74.1396 79.0569 73.5765 76.8023 72.4282 74.835C71.3481 73.1335 70.0673 72.2769 68.5976 72.2769C67.1279 72.2769 65.8884 73.1335 64.8791 74.835C63.805 76.8274 63.2833 79.0713 63.3681 81.3337Z",
  fill: "white",
  fillOpacity: "0.32"
}), /*#__PURE__*/react.createElement("path", {
  d: "M136.091 182.801C135.148 183.665 134.069 184.367 132.898 184.881C131.761 185.4 130.551 185.741 129.31 185.891C128.272 186.031 127.223 186.049 126.181 185.944C125.774 185.944 125.449 185.909 125.207 185.879L124.818 185.767V185.383C124.818 185.165 124.771 184.846 124.753 184.414C124.79 183.378 124.923 182.348 125.148 181.336C125.405 180.114 125.851 178.94 126.471 177.857C127.104 176.729 127.913 175.71 128.867 174.838C129.816 173.969 130.893 173.252 132.06 172.711C133.178 172.182 134.374 171.837 135.602 171.689C136.639 171.549 137.689 171.531 138.73 171.636C139.072 171.647 139.412 171.677 139.751 171.724L140.087 171.813L140.158 172.22C140.183 172.535 140.183 172.851 140.158 173.166C140.16 174.207 140.037 175.244 139.792 176.256C139.518 177.472 139.065 178.641 138.447 179.723C137.82 180.862 137.026 181.899 136.091 182.801Z",
  fill: "var(--online-indicator)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M135.566 175.233C135.69 175.136 135.846 175.089 136.004 175.101C136.161 175.113 136.308 175.183 136.416 175.298C136.488 175.377 136.535 175.474 136.553 175.579C136.57 175.684 136.558 175.791 136.516 175.889C136.469 175.975 136.412 176.054 136.345 176.126L132.373 179.777L133.984 179.818C134.044 179.822 134.102 179.839 134.155 179.866C134.208 179.894 134.255 179.932 134.293 179.979C134.33 180.025 134.358 180.079 134.374 180.137C134.39 180.194 134.394 180.255 134.386 180.314L134.315 180.468C134.282 180.551 134.223 180.621 134.146 180.666C134.068 180.711 133.978 180.729 133.89 180.716L131.434 180.61L128.991 182.825L132.237 182.979C132.297 182.982 132.356 182.998 132.41 183.025C132.464 183.053 132.511 183.091 132.549 183.138C132.588 183.184 132.616 183.238 132.632 183.296C132.649 183.354 132.653 183.415 132.644 183.475V183.652C132.599 183.725 132.537 183.786 132.463 183.829C132.389 183.872 132.305 183.897 132.219 183.9L128.052 183.705L122.841 188.432C122.781 188.484 122.712 188.524 122.638 188.549C122.563 188.574 122.484 188.584 122.405 188.578C122.326 188.572 122.249 188.55 122.179 188.513C122.109 188.477 122.047 188.427 121.997 188.367C121.939 188.315 121.892 188.252 121.861 188.181C121.829 188.11 121.812 188.034 121.812 187.956C121.812 187.878 121.829 187.802 121.861 187.731C121.892 187.66 121.939 187.597 121.997 187.545L127.38 182.713L127.557 178.719C127.586 178.477 127.734 178.359 128.005 178.353C128.277 178.347 128.412 178.5 128.407 178.79L128.265 181.862L130.785 179.617L130.88 177.337C130.881 177.28 130.894 177.224 130.918 177.173C130.942 177.122 130.977 177.077 131.02 177.04C131.063 177.003 131.113 176.976 131.167 176.96C131.221 176.944 131.278 176.939 131.334 176.947C131.389 176.943 131.443 176.951 131.495 176.971C131.546 176.99 131.593 177.02 131.632 177.058C131.671 177.097 131.701 177.143 131.721 177.194C131.741 177.245 131.75 177.3 131.747 177.354L131.671 178.766L135.596 175.222L135.566 175.233Z",
  fill: "black",
  fillOpacity: "0.48"
}), /*#__PURE__*/react.createElement("path", {
  d: "M27.3812 111.275C26.618 111.155 25.8435 111.126 25.0734 111.186C24.43 111.257 24.0936 111.298 24.0582 111.316C23.9945 111.647 23.9727 111.984 23.9932 112.321C23.9933 113.1 24.0704 113.878 24.2234 114.642C24.3947 115.55 24.7074 116.425 25.1501 117.236C25.6043 118.079 26.2038 118.835 26.9208 119.469C27.6276 120.111 28.4227 120.648 29.2818 121.064C30.1151 121.473 31.0074 121.748 31.926 121.879C32.6809 122.002 33.4476 122.036 34.2103 121.98C34.5594 121.96 34.9066 121.914 35.2491 121.844C35.312 121.511 35.3338 121.172 35.314 120.834C35.3113 120.059 35.2262 119.287 35.0602 118.53C34.8697 117.628 34.5497 116.758 34.1099 115.948C33.6481 115.107 33.0497 114.348 32.3392 113.703C31.6377 113.05 30.8418 112.506 29.9783 112.09C29.1578 111.691 28.2825 111.416 27.3812 111.275Z",
  fill: "var(--online-indicator)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M36.9844 124.065C37.2028 124.254 37.3976 124.272 37.5746 124.107C37.6182 124.066 37.653 124.016 37.6768 123.961C37.7005 123.907 37.7128 123.847 37.7128 123.788C37.7128 123.728 37.7005 123.669 37.6768 123.614C37.653 123.559 37.6182 123.51 37.5746 123.469L34.0627 120.125L33.9919 117.129C33.9919 116.929 33.862 116.828 33.6319 116.828C33.5843 116.822 33.5361 116.827 33.4904 116.841C33.4447 116.855 33.4025 116.879 33.3668 116.911C33.331 116.943 33.3025 116.982 33.283 117.026C33.2636 117.07 33.2537 117.117 33.2541 117.165L33.3426 119.386L31.749 117.862V116.09C31.7501 116.04 31.7407 115.991 31.7214 115.946C31.7021 115.9 31.6733 115.859 31.637 115.826C31.6008 115.792 31.5578 115.767 31.5109 115.751C31.4641 115.736 31.4144 115.73 31.3653 115.735C31.318 115.729 31.27 115.734 31.2244 115.747C31.1788 115.761 31.1366 115.785 31.1004 115.816C31.0643 115.847 31.0351 115.885 31.0147 115.928C30.9942 115.971 30.983 116.018 30.9817 116.066V117.141L28.3433 114.589C28.3095 114.551 28.2682 114.521 28.2219 114.5C28.1756 114.479 28.1255 114.469 28.0747 114.469C28.024 114.469 27.9739 114.479 27.9276 114.5C27.8814 114.521 27.84 114.551 27.8062 114.589C27.7389 114.639 27.6889 114.708 27.6635 114.788C27.638 114.867 27.6384 114.953 27.6645 115.032L27.7413 115.186L30.3501 117.685H29.4057C29.3147 117.705 29.2333 117.756 29.1753 117.829C29.1173 117.902 29.0861 117.993 29.087 118.087V118.211C29.1165 118.278 29.1655 118.335 29.2276 118.373C29.2897 118.412 29.362 118.432 29.4353 118.429L31.1233 118.394L32.6639 119.948H30.598C30.4971 119.947 30.3999 119.986 30.3264 120.055C30.2529 120.124 30.2086 120.219 30.2026 120.32L30.2734 120.479C30.2928 120.536 30.3306 120.585 30.3808 120.618C30.431 120.651 30.4908 120.667 30.5508 120.662H33.4371L36.9785 124.059L36.9844 124.065Z",
  fill: "black",
  fillOpacity: "0.48"
}), /*#__PURE__*/react.createElement("path", {
  d: "M174.864 65.0161C174.767 64.5516 174.613 64.1011 174.404 63.675C174.227 63.3264 174.126 63.1433 174.115 63.1256C173.911 63.1618 173.713 63.2233 173.524 63.3087C173.08 63.4828 172.653 63.6984 172.25 63.9527C171.768 64.2474 171.337 64.6185 170.975 65.0516C170.596 65.4952 170.296 66.0008 170.089 66.5462C169.878 67.0958 169.747 67.6727 169.7 68.2595C169.652 68.8255 169.692 69.3954 169.818 69.9492C169.913 70.408 170.062 70.8541 170.26 71.2784C170.348 71.4828 170.45 71.6803 170.567 71.8692C170.771 71.833 170.969 71.7715 171.158 71.6861C171.601 71.5132 172.024 71.2933 172.421 71.0303C172.893 70.7228 173.318 70.3487 173.684 69.9197C174.059 69.476 174.362 68.9755 174.581 68.4368C174.799 67.8929 174.934 67.3194 174.982 66.7353C175.033 66.1596 174.993 65.5794 174.864 65.0161Z",
  fill: "var(--online-indicator)"
}), /*#__PURE__*/react.createElement("path", {
  d: "M169.682 73.3167C169.623 73.4821 169.652 73.5944 169.788 73.6594C169.821 73.6761 169.856 73.6861 169.892 73.6886C169.929 73.6911 169.965 73.6861 169.999 73.6739C170.034 73.6617 170.065 73.6426 170.092 73.6177C170.118 73.5929 170.14 73.5628 170.154 73.5294L171.293 70.7881L172.981 70.091C173.008 70.0819 173.031 70.0671 173.051 70.0476C173.071 70.028 173.085 70.0042 173.094 69.978C173.104 69.9519 173.107 69.924 173.103 69.8965C173.1 69.869 173.091 69.8426 173.076 69.8192C173.069 69.79 173.057 69.7626 173.039 69.7388C173.021 69.715 172.998 69.6954 172.971 69.6813C172.945 69.6673 172.916 69.6592 172.886 69.6575C172.856 69.6558 172.826 69.6606 172.798 69.6715L171.553 70.2151L172.072 68.9685L173.076 68.5786C173.104 68.5684 173.13 68.5524 173.151 68.5315C173.173 68.5106 173.19 68.4853 173.201 68.4574C173.212 68.4295 173.217 68.3996 173.216 68.3695C173.215 68.3395 173.207 68.3101 173.194 68.2832C173.187 68.2553 173.174 68.2291 173.156 68.2065C173.138 68.184 173.115 68.1654 173.09 68.152C173.064 68.1387 173.036 68.1308 173.007 68.129C172.978 68.1272 172.949 68.1314 172.922 68.1414L172.332 68.3718L173.206 66.3041C173.22 66.2765 173.229 66.2461 173.231 66.215C173.232 66.1839 173.228 66.1527 173.216 66.1236C173.205 66.0945 173.188 66.0682 173.166 66.0463C173.144 66.0245 173.117 66.0076 173.088 65.9969C173.046 65.9671 172.997 65.951 172.946 65.951C172.895 65.951 172.846 65.9671 172.804 65.9969L172.733 66.0737L171.884 68.1178L171.677 67.5742C171.643 67.5286 171.596 67.4953 171.541 67.4793C171.487 67.4633 171.429 67.4655 171.376 67.4857H171.305C171.273 67.517 171.251 67.5578 171.242 67.6022C171.234 67.6466 171.239 67.6926 171.258 67.7337L171.647 68.6908L171.099 69.9138L170.632 68.7322C170.611 68.675 170.568 68.6283 170.513 68.6018C170.458 68.5754 170.395 68.5713 170.337 68.5904L170.266 68.6672C170.236 68.6887 170.215 68.7201 170.206 68.7559C170.198 68.7917 170.202 68.8294 170.219 68.8622L170.862 70.5105L169.682 73.2753V73.3167Z",
  fill: "black",
  fillOpacity: "0.48"
})), /*#__PURE__*/react.createElement("defs", null, /*#__PURE__*/react.createElement("clipPath", {
  id: "clip0"
}, /*#__PURE__*/react.createElement("rect", {
  width: "175",
  height: "207",
  fill: "white"
}))));
/* harmony default export */ const balloon_illustration = (BalloonIllustration);
// EXTERNAL MODULE: ./actions/views/lhs.js
var lhs = __webpack_require__(2535);
// EXTERNAL MODULE: ./selectors/views/browser.ts
var browser = __webpack_require__(61958);
// EXTERNAL MODULE: ./components/formatted_markdown_message.jsx
var formatted_markdown_message = __webpack_require__(45288);
// EXTERNAL MODULE: ./components/tutorial/tutorial_tip_legacy/index.ts + 2 modules
var tutorial_tip_legacy = __webpack_require__(81961);
// EXTERNAL MODULE: ./components/shortcut_key/index.ts + 1 modules
var shortcut_key = __webpack_require__(25779);
// EXTERNAL MODULE: ./components/tutorial/tutorial_tip_legacy/hooks.ts
var tutorial_tip_legacy_hooks = __webpack_require__(43538);
;// CONCATENATED MODULE: ./components/crt_tour/crt_list_tutorial_tip/crt_list_tutorial_tip.tsx

// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.










const UpShortcut = () => /*#__PURE__*/react.createElement(shortcut_key/* ShortcutKey */.j, {
  variant: shortcut_key/* ShortcutKeyVariant.TutorialTip */.S.TutorialTip
}, 'UP');
const DownShortcut = () => /*#__PURE__*/react.createElement(shortcut_key/* ShortcutKey */.j, {
  variant: shortcut_key/* ShortcutKeyVariant.TutorialTip */.S.TutorialTip
}, 'DOWN');

const CRTListTutorialTip = _ref => {
  let {
    autoTour
  } = _ref;
  const isMobileView = (0,es.useSelector)(state => (0,browser/* getIsMobileView */.E7)(state));
  const dispatch = (0,es.useDispatch)();

  const onPrevNavigateTo = () => dispatch((0,lhs/* open */.bA)());

  const title = /*#__PURE__*/react.createElement(message/* default */.Z, {
    id: "tutorial_threads.list.title",
    defaultMessage: 'Threads List'
  });
  const screen = /*#__PURE__*/react.createElement(react.Fragment, null, /*#__PURE__*/react.createElement("p", null, /*#__PURE__*/react.createElement(formatted_markdown_message/* default */.Z, {
    id: "tutorial_threads.list.description-p1",
    defaultMessage: 'Here you’ll see a preview of all threads you’re following or participating in. Clicking on a thread in this list will open the full thread on the right.'
  })), /*#__PURE__*/react.createElement("p", null, /*#__PURE__*/react.createElement(message/* default */.Z, {
    id: "tutorial_threads.list.description-p2",
    defaultMessage: 'Use {upKey} / {downKey} to navigate the thread list.',
    values: {
      upKey: /*#__PURE__*/react.createElement(UpShortcut, null),
      downKey: /*#__PURE__*/react.createElement(DownShortcut, null)
    }
  })));
  const punchOutIds = isMobileView ? ['tutorial-threads-mobile-list', 'tutorial-threads-mobile-header'] : ['threads-list-container'];
  return /*#__PURE__*/react.createElement(tutorial_tip_legacy/* default */.Z, {
    title: title,
    placement: "right",
    showOptOut: false,
    step: constants/* Constants.CrtTutorialSteps.LIST_POPOVER */.gT.CrtTutorialSteps.LIST_POPOVER,
    tutorialCategory: constants/* Preferences.CRT_TUTORIAL_STEP */.uP.CRT_TUTORIAL_STEP,
    stopPropagation: true,
    screen: screen,
    overlayClass: "tip-overlay--threads-list",
    autoTour: autoTour,
    onPrevNavigateTo: onPrevNavigateTo,
    punchOut: (0,tutorial_tip_legacy_hooks/* useMeasurePunchoutsDeprecated */._)(punchOutIds, [])
  });
};

CRTListTutorialTip.propTypes = {
  autoTour: (prop_types_default()).bool.isRequired
};
/* harmony default export */ const crt_list_tutorial_tip = (CRTListTutorialTip);
// EXTERNAL MODULE: ./packages/mattermost-redux/src/selectors/entities/preferences.ts
var preferences = __webpack_require__(89754);
;// CONCATENATED MODULE: ./components/crt_tour/crt_unread_tutorial_tip/crt_unread_tutorial_tip.tsx

// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.







const CRTUnreadTutorialTip = _ref => {
  let {
    autoTour
  } = _ref;
  const title = /*#__PURE__*/react.createElement(message/* default */.Z, {
    id: "tutorial_threads.unread.title",
    defaultMessage: 'Unread threads'
  });
  const screen = /*#__PURE__*/react.createElement("p", null, /*#__PURE__*/react.createElement(formatted_markdown_message/* default */.Z, {
    id: "tutorial_threads.unread.description",
    defaultMessage: 'You can switch to **Unreads** to show only threads that are unread.'
  }));
  return /*#__PURE__*/react.createElement(tutorial_tip_legacy/* default */.Z, {
    title: title,
    placement: "bottom",
    showOptOut: false,
    step: constants/* Constants.CrtTutorialSteps.UNREAD_POPOVER */.gT.CrtTutorialSteps.UNREAD_POPOVER,
    tutorialCategory: constants/* Preferences.CRT_TUTORIAL_STEP */.uP.CRT_TUTORIAL_STEP,
    screen: screen,
    overlayClass: "tip-overlay--threads-unread",
    autoTour: autoTour,
    punchOut: (0,tutorial_tip_legacy_hooks/* useMeasurePunchoutsDeprecated */._)(['threads-list-unread-button'], [])
  });
};

CRTUnreadTutorialTip.propTypes = {
  autoTour: (prop_types_default()).bool.isRequired
};
/* harmony default export */ const crt_unread_tutorial_tip = (CRTUnreadTutorialTip);
// EXTERNAL MODULE: ./node_modules/react-virtualized-auto-sizer/dist/index.esm.js
var index_esm = __webpack_require__(11728);
// EXTERNAL MODULE: ./node_modules/react-window-infinite-loader/dist/index.esm.js
var dist_index_esm = __webpack_require__(65691);
// EXTERNAL MODULE: ./node_modules/react-window/dist/index.esm.js
var react_window_dist_index_esm = __webpack_require__(74061);
// EXTERNAL MODULE: ./components/no_results_indicator/types.ts
var types = __webpack_require__(13516);
// EXTERNAL MODULE: ./components/search_shortcut/search_shortcut.tsx
var search_shortcut = __webpack_require__(47485);
// EXTERNAL MODULE: ./node_modules/core-js/modules/es.regexp.to-string.js
var es_regexp_to_string = __webpack_require__(39714);
;// CONCATENATED MODULE: ./components/common/svg_images_components/search_hint_svg.tsx


// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.


const SearchHintSVG = _ref => {
  let {
    width,
    height
  } = _ref;
  return /*#__PURE__*/react.createElement("svg", {
    width: (width === null || width === void 0 ? void 0 : width.toString()) || '72',
    height: (height === null || height === void 0 ? void 0 : height.toString()) || '48',
    viewBox: "0 0 72 48",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg"
  }, /*#__PURE__*/react.createElement("g", {
    clipPath: "url(#clip0_410_25534)"
  }, /*#__PURE__*/react.createElement("path", {
    d: "M35.1353 12.5745H4.8754C4.32086 12.5726 3.77137 12.6798 3.25832 12.8898C2.74526 13.0998 2.27869 13.4087 1.88524 13.7986C1.49178 14.1886 1.17916 14.6521 0.965209 15.1627C0.751261 15.6732 0.640179 16.2208 0.638306 16.7742V35.9654C0.640179 36.5188 0.751261 37.0664 0.965209 37.577C1.17916 38.0875 1.49178 38.551 1.88524 38.941C2.27869 39.331 2.74526 39.6398 3.25832 39.8498C3.77137 40.0599 4.32086 40.167 4.8754 40.1651H9.34115V47.3489L16.0398 40.1651H35.1246C35.6791 40.167 36.2286 40.0599 36.7417 39.8498C37.2547 39.6398 37.7213 39.331 38.1147 38.941C38.5082 38.551 38.8208 38.0875 39.0348 37.577C39.2487 37.0664 39.3598 36.5188 39.3617 35.9654V16.7742C39.3579 15.6585 38.9108 14.5897 38.1185 13.8024C37.3262 13.0151 36.2534 12.5735 35.1353 12.5745Z",
    fill: "var(--away-indicator)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M16.0398 40.1651H35.1246C35.6791 40.167 36.2286 40.0598 36.7417 39.8498C37.2547 39.6397 37.7213 39.3309 38.1148 38.9409C38.5082 38.5509 38.8208 38.0874 39.0348 37.5769C39.2487 37.0663 39.3598 36.5187 39.3617 35.9653V24.3145C39.3617 24.3145 38.0291 35.0883 37.7897 36.0367C37.5504 36.985 37.0752 38.4039 34.8245 38.6392C32.5738 38.8745 16.0398 40.1651 16.0398 40.1651Z",
    fill: "#CC8F00"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M9.66973 23.6263C10.2089 23.6263 10.7359 23.7859 11.1842 24.0848C11.6324 24.3837 11.9818 24.8085 12.1881 25.3056C12.3944 25.8026 12.4484 26.3496 12.3432 26.8772C12.2381 27.4049 11.9785 27.8896 11.5972 28.27C11.216 28.6504 10.7303 28.9095 10.2015 29.0145C9.67275 29.1194 9.12467 29.0656 8.62657 28.8597C8.12848 28.6538 7.70276 28.3051 7.40323 27.8578C7.10371 27.4105 6.94383 26.8845 6.94383 26.3465C6.94336 25.9892 7.01355 25.6353 7.15038 25.305C7.2872 24.9748 7.48797 24.6747 7.74118 24.422C7.9944 24.1693 8.29509 23.969 8.62602 23.8324C8.95695 23.6959 9.31163 23.6259 9.66973 23.6263Z",
    fill: "var(--center-channel-bg)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M20.0056 23.6263C20.5447 23.6263 21.0717 23.7859 21.52 24.0848C21.9683 24.3837 22.3177 24.8085 22.524 25.3056C22.7303 25.8026 22.7843 26.3496 22.6791 26.8772C22.5739 27.4049 22.3143 27.8896 21.9331 28.27C21.5518 28.6504 21.0661 28.9095 20.5374 29.0145C20.0086 29.1194 19.4605 29.0656 18.9624 28.8597C18.4643 28.6538 18.0386 28.3051 17.7391 27.8578C17.4396 27.4105 17.2797 26.8845 17.2797 26.3465C17.2792 25.9892 17.3494 25.6353 17.4862 25.305C17.6231 24.9748 17.8238 24.6747 18.077 24.422C18.3303 24.1693 18.6309 23.969 18.9619 23.8324C19.2928 23.6959 19.6475 23.6259 20.0056 23.6263Z",
    fill: "var(--center-channel-bg)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M30.33 23.6263C30.8693 23.6256 31.3967 23.7846 31.8454 24.083C32.2942 24.3815 32.6441 24.8061 32.851 25.3031C33.0579 25.8001 33.1123 26.3471 33.0076 26.875C32.9028 27.4029 32.6435 27.888 32.2624 28.2688C31.8813 28.6495 31.3956 28.909 30.8667 29.0142C30.3379 29.1195 29.7896 29.0658 29.2913 28.86C28.793 28.6542 28.3671 28.3056 28.0674 27.8582C27.7677 27.4108 27.6077 26.8847 27.6077 26.3465C27.6073 25.9895 27.6773 25.6358 27.8139 25.3058C27.9505 24.9758 28.151 24.6759 28.4038 24.4233C28.6566 24.1706 28.9569 23.9702 29.2874 23.8335C29.618 23.6967 29.9722 23.6263 30.33 23.6263Z",
    fill: "var(--center-channel-bg)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M3.7787 20.6104C4.08616 19.5244 4.60589 18.5098 5.30794 17.625C6.00999 16.7403 6.88049 16.0028 7.86932 15.4552C7.92789 15.4257 7.97498 15.3776 8.00322 15.3184C8.03145 15.2593 8.03923 15.1925 8.02534 15.1285C8.01145 15.0645 7.97668 15.0069 7.92646 14.9647C7.87625 14.9225 7.81343 14.8982 7.74785 14.8955C5.84009 14.7814 1.98883 15.1878 3.19637 20.5819C3.20911 20.6477 3.24362 20.7074 3.29437 20.7513C3.34513 20.7953 3.4092 20.821 3.47632 20.8242C3.54343 20.8275 3.60972 20.8082 3.66454 20.7695C3.71937 20.7307 3.75956 20.6747 3.7787 20.6104Z",
    fill: "#FFD470"
  }), /*#__PURE__*/react.createElement("path", {
    opacity: "0.4",
    d: "M32.3334 7.28991C34.7785 4.81371 37.8644 3.53829 41.5849 3.46985C45.237 3.53829 48.3042 4.80749 50.7866 7.28991C53.2628 9.77233 54.5383 12.8396 54.6067 16.4917C54.5383 20.2122 53.269 23.2919 50.7866 25.7432C48.3042 28.1883 45.237 29.4824 41.5849 29.6068C37.8644 29.4761 34.7847 28.1883 32.3334 25.7432C29.8883 23.2919 28.6004 20.206 28.4698 16.4854C28.6004 12.8334 29.8883 9.76611 32.3334 7.28991Z",
    fill: "var(--center-channel-bg)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M53.5706 14.1678C52.464 10.5721 50.3691 7.88057 47.2828 6.09868C44.1964 4.31679 40.8181 3.84832 37.1508 4.68784C35.1455 5.20001 33.3983 6.08537 31.8951 7.34293C33.576 5.5084 35.7082 4.23809 38.2863 3.52882C41.9535 2.68929 45.3485 3.16738 48.4626 4.9653C51.5767 6.76321 53.6882 9.46433 54.7948 13.06C55.4751 15.6505 55.4412 18.1323 54.6873 20.502C53.939 22.875 52.5957 24.8683 50.6551 26.4733C52.1121 24.8855 53.1147 23.0208 53.6603 20.8706C54.2005 18.7172 54.1703 16.4876 53.5706 14.1678Z",
    fill: "black",
    fillOpacity: "0.4"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M57.6194 15.4776C57.3584 11.6178 55.7919 8.22807 52.9264 5.30846C49.663 2.19708 45.8961 0.638306 41.6257 0.638306C37.3554 0.638306 33.5947 2.19708 30.3313 5.30846C27.2047 8.55591 25.6383 12.2982 25.6383 16.5477C25.6383 20.7973 27.2047 24.5458 30.3313 27.787C33.3274 30.6448 36.7524 32.185 40.6001 32.4077C44.4478 32.6366 47.9971 31.5788 51.2605 29.2468L53.7034 31.6778L56.83 28.5664L54.3871 26.1355C56.7989 22.8942 57.8743 19.3375 57.6194 15.4776ZM50.8689 25.6963C48.3887 28.1272 45.3242 29.4139 41.6755 29.5376C37.9583 29.4077 34.8814 28.1272 32.4323 25.6963C29.9895 23.2653 28.7028 20.1973 28.5722 16.4983C28.7028 12.8673 29.9895 9.81778 32.4323 7.34971C34.8752 4.88783 37.9583 3.61978 41.6755 3.55174C45.3242 3.61978 48.3887 4.88165 50.8689 7.34971C53.3428 9.81778 54.6171 12.8673 54.6855 16.4983C54.6171 20.1973 53.3491 23.2653 50.8689 25.6963Z",
    fill: "#BABEC9"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M67.6182 46.4679C66.8121 46.6983 66.2153 46.6174 65.8276 46.225L53.1453 32.3065C52.7577 31.9142 52.5977 31.4098 52.6592 30.787C52.7207 30.1643 53.1084 29.4668 53.8222 28.6821C54.5975 27.9659 55.2867 27.5549 55.9021 27.4553C56.5174 27.3556 57.0159 27.5362 57.4035 27.9971L71.2488 40.832C71.6364 41.2243 71.698 41.8097 71.4395 42.5944C71.1811 43.3791 70.695 44.1637 69.9873 44.9484C69.2181 45.7331 68.4243 46.2375 67.6182 46.4679Z",
    fill: "var(--away-indicator)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M68.5465 38.3191L63.3621 43.5465L57.1116 36.6856L61.6961 32.0027L68.5465 38.3191Z",
    fill: "#7A5600"
  })), /*#__PURE__*/react.createElement("defs", null, /*#__PURE__*/react.createElement("clipPath", {
    id: "clip0_410_25534"
  }, /*#__PURE__*/react.createElement("rect", {
    width: "72",
    height: "48",
    fill: "var(--center-channel-bg)"
  }))));
};

SearchHintSVG.propTypes = {
  width: (prop_types_default()).number,
  height: (prop_types_default()).number
};
/* harmony default export */ const search_hint_svg = (SearchHintSVG);
// EXTERNAL MODULE: ./node_modules/redux/es/redux.js + 1 modules
var redux = __webpack_require__(97779);
// EXTERNAL MODULE: ./packages/mattermost-redux/src/selectors/entities/channels.ts
var entities_channels = __webpack_require__(9276);
// EXTERNAL MODULE: ./node_modules/core-js/modules/es.array.reverse.js
var es_array_reverse = __webpack_require__(65069);
// EXTERNAL MODULE: ./node_modules/core-js/modules/es.array.flat-map.js
var es_array_flat_map = __webpack_require__(86535);
// EXTERNAL MODULE: ./node_modules/core-js/modules/es.array.unscopables.flat-map.js
var es_array_unscopables_flat_map = __webpack_require__(86587);
// EXTERNAL MODULE: ./packages/mattermost-redux/src/actions/users.ts
var actions_users = __webpack_require__(33383);
// EXTERNAL MODULE: ./packages/mattermost-redux/src/constants/index.ts + 10 modules
var src_constants = __webpack_require__(33407);
// EXTERNAL MODULE: ./components/widgets/badges/badge.tsx
var badge = __webpack_require__(17189);
// EXTERNAL MODULE: ./components/timestamp/index.ts + 4 modules
var timestamp = __webpack_require__(49801);
// EXTERNAL MODULE: ./components/widgets/users/avatars/index.ts + 1 modules
var avatars = __webpack_require__(12770);
// EXTERNAL MODULE: ./components/markdown/index.ts + 1 modules
var markdown = __webpack_require__(64852);
// EXTERNAL MODULE: ./components/threading/common/options.ts
var options = __webpack_require__(96329);
// EXTERNAL MODULE: ./actions/post_actions.jsx + 2 modules
var post_actions = __webpack_require__(34959);
// EXTERNAL MODULE: ./utils/url.tsx + 1 modules
var url = __webpack_require__(12266);
// EXTERNAL MODULE: ./utils/i18n.tsx
var i18n = __webpack_require__(87993);
// EXTERNAL MODULE: ./components/widgets/menu/menu.tsx + 13 modules
var menu = __webpack_require__(87760);
// EXTERNAL MODULE: ./components/widgets/menu/menu_wrapper.tsx + 1 modules
var menu_wrapper = __webpack_require__(38122);
;// CONCATENATED MODULE: ./components/threading/global_threads/thread_menu/thread_menu.tsx


// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.
















function ThreadMenu(_ref) {
  let {
    threadId,
    isFollowing = false,
    unreadTimestamp,
    hasUnreads,
    children
  } = _ref;
  const {
    formatMessage
  } = (0,useIntl/* default */.Z)();
  const dispatch = (0,es.useDispatch)();
  const {
    params: {
      team
    },
    currentTeamId,
    currentUserId,
    goToInChannel
  } = (0,hooks/* useThreadRouting */.Q)();
  const isSaved = (0,es.useSelector)(state => (0,preferences/* get */.U2)(state, src_constants/* Preferences.CATEGORY_FLAGGED_POST */.uP.CATEGORY_FLAGGED_POST, threadId, null) != null, es.shallowEqual);
  const handleReadUnread = (0,react.useCallback)(() => {
    const lastViewedAt = hasUnreads ? Date.now() : unreadTimestamp;
    dispatch((0,views_threads/* manuallyMarkThreadAsUnread */.Fc)(threadId, lastViewedAt));

    if (hasUnreads) {
      dispatch((0,actions_threads/* updateThreadRead */.bl)(currentUserId, currentTeamId, threadId, Date.now()));
    } else {
      dispatch((0,actions_threads/* markThreadAsUnread */.Zj)(currentUserId, currentTeamId, threadId, threadId));
    }
  }, [currentUserId, currentTeamId, threadId, hasUnreads, actions_threads/* updateThreadRead */.bl, unreadTimestamp]);
  return /*#__PURE__*/react.createElement(menu_wrapper/* default */.Z, {
    stopPropagationOnToggle: true
  }, children, /*#__PURE__*/react.createElement(menu/* default */.Z, {
    ariaLabel: '',
    openLeft: true
  }, /*#__PURE__*/react.createElement(menu/* default.ItemAction */.Z.ItemAction, Object.assign({}, isFollowing ? {
    text: formatMessage({
      id: (0,i18n.t)('threading.threadMenu.unfollow'),
      defaultMessage: 'Unfollow thread'
    }),
    extraText: formatMessage({
      id: (0,i18n.t)('threading.threadMenu.unfollowExtra'),
      defaultMessage: 'You won’t be notified about replies'
    })
  } : {
    text: formatMessage({
      id: (0,i18n.t)('threading.threadMenu.follow'),
      defaultMessage: 'Follow thread'
    }),
    extraText: formatMessage({
      id: (0,i18n.t)('threading.threadMenu.followExtra'),
      defaultMessage: 'You will be notified about replies'
    })
  }, {
    onClick: (0,react.useCallback)(() => {
      dispatch((0,actions_threads/* setThreadFollow */.NC)(currentUserId, currentTeamId, threadId, !isFollowing));
    }, [currentUserId, currentTeamId, threadId, isFollowing, actions_threads/* setThreadFollow */.NC])
  })), /*#__PURE__*/react.createElement(menu/* default.ItemAction */.Z.ItemAction, {
    text: formatMessage({
      id: (0,i18n.t)('threading.threadMenu.openInChannel'),
      defaultMessage: 'Open in channel'
    }),
    onClick: (0,react.useCallback)(() => {
      goToInChannel(threadId);
    }, [threadId])
  }), /*#__PURE__*/react.createElement(menu/* default.ItemAction */.Z.ItemAction, {
    text: formatMessage(hasUnreads ? {
      id: (0,i18n.t)('threading.threadMenu.markRead'),
      defaultMessage: 'Mark as read'
    } : {
      id: (0,i18n.t)('threading.threadMenu.markUnread'),
      defaultMessage: 'Mark as unread'
    }),
    onClick: handleReadUnread
  }), /*#__PURE__*/react.createElement(menu/* default.ItemAction */.Z.ItemAction, {
    text: formatMessage(isSaved ? {
      id: (0,i18n.t)('threading.threadMenu.unsave'),
      defaultMessage: 'Unsave'
    } : {
      id: (0,i18n.t)('threading.threadMenu.save'),
      defaultMessage: 'Save'
    }),
    onClick: (0,react.useCallback)(() => {
      dispatch(isSaved ? (0,post_actions/* unflagPost */.xk)(threadId) : (0,post_actions/* flagPost */.co)(threadId));
    }, [threadId, isSaved])
  }), /*#__PURE__*/react.createElement(menu/* default.ItemAction */.Z.ItemAction, {
    text: formatMessage({
      id: (0,i18n.t)('threading.threadMenu.copy'),
      defaultMessage: 'Copy link'
    }),
    onClick: (0,react.useCallback)(() => {
      (0,utils/* copyToClipboard */.vQ)("".concat((0,url/* getSiteURL */.fO)(), "/").concat(team, "/pl/").concat(threadId));
    }, [team, threadId])
  })));
}

ThreadMenu.propTypes = {
  isFollowing: (prop_types_default()).bool,
  hasUnreads: (prop_types_default()).bool.isRequired,
  children: (prop_types_default()).node.isRequired,
  unreadTimestamp: (prop_types_default()).number.isRequired
};

function areEqual(prevProps, nextProps) {
  return prevProps.threadId === nextProps.threadId && prevProps.isFollowing === nextProps.isFollowing && prevProps.unreadTimestamp === nextProps.unreadTimestamp && prevProps.hasUnreads === nextProps.hasUnreads;
}

/* harmony default export */ const thread_menu = (/*#__PURE__*/(0,react.memo)(ThreadMenu, areEqual));
;// CONCATENATED MODULE: ./components/threading/global_threads/thread_menu/index.ts
// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.

// EXTERNAL MODULE: ./components/threading/global_threads/thread_item/attachments/index.tsx + 3 modules
var attachments = __webpack_require__(82308);
;// CONCATENATED MODULE: ./components/threading/global_threads/thread_item/thread_item.tsx






// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.
























const markdownPreviewOptions = {
  singleline: true,
  mentionHighlight: false,
  atMentions: false
};

function ThreadItem(_ref) {
  var _post$props, _post$metadata;

  let {
    channel,
    currentRelativeTeamUrl,
    displayName,
    isSelected,
    post,
    postsInThread,
    style,
    thread,
    threadId,
    isFirstThreadInList
  } = _ref;
  const dispatch = (0,es.useDispatch)();
  const {
    select,
    goToInChannel
  } = (0,hooks/* useThreadRouting */.Q)();
  const {
    formatMessage
  } = (0,useIntl/* default */.Z)();
  const isMobileView = (0,es.useSelector)(browser/* getIsMobileView */.E7);
  const currentUserId = (0,es.useSelector)(users/* getCurrentUserId */.rc);
  const tipStep = (0,es.useSelector)(state => (0,preferences/* getInt */.wS)(state, constants/* Preferences.CRT_TUTORIAL_STEP */.uP.CRT_TUTORIAL_STEP, currentUserId));
  const tutorialTipAutoTour = (0,es.useSelector)(state => (0,preferences/* getInt */.wS)(state, constants/* Preferences.CRT_TUTORIAL_AUTO_TOUR_STATUS */.uP.CRT_TUTORIAL_AUTO_TOUR_STATUS, currentUserId, constants/* Constants.AutoTourStatus.ENABLED */.gT.AutoTourStatus.ENABLED)) === constants/* Constants.AutoTourStatus.ENABLED */.gT.AutoTourStatus.ENABLED;
  const showListTutorialTip = tipStep === constants/* CrtTutorialSteps.LIST_POPOVER */.WC.LIST_POPOVER;
  const msgDeleted = formatMessage({
    id: 'post_body.deleted',
    defaultMessage: '(message deleted)'
  });
  const postAuthor = ((_post$props = post.props) === null || _post$props === void 0 ? void 0 : _post$props.override_username) || displayName;
  (0,react.useEffect)(() => {
    if (channel !== null && channel !== void 0 && channel.teammate_id) {
      dispatch((0,actions_users/* getMissingProfilesByIds */.C4)([channel.teammate_id]));
    }
  }, [channel === null || channel === void 0 ? void 0 : channel.teammate_id]);
  (0,react.useEffect)(() => {
    if (!channel && thread !== null && thread !== void 0 && thread.post.channel_id) {
      dispatch((0,channels/* getChannel */.mr)(thread.post.channel_id));
    }
  }, [channel, thread === null || thread === void 0 ? void 0 : thread.post.channel_id]);
  const participantIds = (0,react.useMemo)(() => {
    const ids = ((thread === null || thread === void 0 ? void 0 : thread.participants) || []).flatMap(_ref2 => {
      let {
        id
      } = _ref2;

      if (id === post.user_id) {
        return [];
      }

      return id;
    }).reverse();
    return [post.user_id, ...ids];
  }, [thread === null || thread === void 0 ? void 0 : thread.participants]);
  const selectHandler = (0,react.useCallback)(() => select(threadId), [threadId]);
  const imageProps = (0,react.useMemo)(() => ({
    onImageHeightChanged: () => {},
    onImageLoaded: () => {}
  }), []);
  const goToInChannelHandler = (0,react.useCallback)(e => {
    e.stopPropagation();
    goToInChannel(threadId);
  }, [threadId]);
  const handleFormattedTextClick = (0,react.useCallback)(e => {
    utils/* handleFormattedTextClick */.QJ(e, currentRelativeTeamUrl);
  }, [currentRelativeTeamUrl]);

  if (!thread || !post) {
    return null;
  }

  const {
    unread_replies: newReplies,
    unread_mentions: newMentions,
    last_reply_at: lastReplyAt,
    reply_count: totalReplies,
    is_following: isFollowing
  } = thread;
  let unreadTimestamp = post.edit_at || post.create_at; // if we have the whole thread, get the posts in it, sorted from newest to oldest.
  // Last post - root post, second to last post - oldest reply. Use that timestamp

  if (postsInThread.length > 1) {
    const p = postsInThread[postsInThread.length - 2];
    unreadTimestamp = p.edit_at || p.create_at;
  }

  return /*#__PURE__*/react.createElement("article", {
    style: style,
    className: classnames_default()('ThreadItem', {
      'has-unreads': newReplies,
      'is-selected': isSelected
    }),
    tabIndex: 0,
    id: isFirstThreadInList ? 'tutorial-threads-mobile-list' : '',
    onClick: selectHandler
  }, /*#__PURE__*/react.createElement("h1", null, Boolean(newMentions || newReplies) && /*#__PURE__*/react.createElement("div", {
    className: "indicator"
  }, newMentions ? /*#__PURE__*/react.createElement("div", {
    className: classnames_default()('dot-mentions', {
      over: newMentions > 99
    })
  }, Math.min(newMentions, 99), newMentions > 99 && '+') : /*#__PURE__*/react.createElement("div", {
    className: "dot-unreads"
  })), /*#__PURE__*/react.createElement("span", null, postAuthor), Boolean(channel) && /*#__PURE__*/react.createElement(badge/* default */.Z, {
    className: classnames_default()({
      Badge__hidden: postAuthor === (channel === null || channel === void 0 ? void 0 : channel.display_name)
    }),
    onClick: goToInChannelHandler
  }, channel === null || channel === void 0 ? void 0 : channel.display_name), /*#__PURE__*/react.createElement(timestamp/* default */.ZP, Object.assign({}, options/* THREADING_TIME */._, {
    className: "alt-hidden",
    value: lastReplyAt
  }))), /*#__PURE__*/react.createElement("div", {
    className: "menu-anchor alt-visible"
  }, /*#__PURE__*/react.createElement(thread_menu, {
    threadId: threadId,
    isFollowing: isFollowing !== null && isFollowing !== void 0 ? isFollowing : false,
    hasUnreads: Boolean(newReplies),
    unreadTimestamp: unreadTimestamp
  }, /*#__PURE__*/react.createElement(simple_tooltip/* default */.Z, {
    id: "threadActionMenu",
    content: /*#__PURE__*/react.createElement(message/* default */.Z, {
      id: "threading.threadItem.menu",
      defaultMessage: "Actions"
    })
  }, /*#__PURE__*/react.createElement(common_button/* default */.Z, {
    className: "Button___icon"
  }, /*#__PURE__*/react.createElement("i", {
    className: "Icon icon-dots-vertical"
  }))))), /*#__PURE__*/react.createElement("div", {
    "aria-readonly": "true",
    className: "preview",
    dir: "auto",
    tabIndex: 0,
    onClick: handleFormattedTextClick
  }, post.message ? /*#__PURE__*/react.createElement(markdown/* default */.Z, {
    message: post.state === src_constants/* Posts.POST_DELETED */.V8.POST_DELETED ? msgDeleted : post.message,
    options: markdownPreviewOptions,
    imagesMetadata: (post === null || post === void 0 ? void 0 : post.metadata) && (post === null || post === void 0 ? void 0 : (_post$metadata = post.metadata) === null || _post$metadata === void 0 ? void 0 : _post$metadata.images),
    imageProps: imageProps
  }) : /*#__PURE__*/react.createElement(attachments/* default */.Z, {
    post: post
  })), /*#__PURE__*/react.createElement("div", {
    className: "activity"
  }, participantIds !== null && participantIds !== void 0 && participantIds.length ? /*#__PURE__*/react.createElement(avatars/* default */.Z, {
    userIds: participantIds,
    size: "xs"
  }) : null, Boolean(totalReplies) && /*#__PURE__*/react.createElement(react.Fragment, null, newReplies ? /*#__PURE__*/react.createElement(message/* default */.Z, {
    id: "threading.numNewReplies",
    defaultMessage: "{newReplies, plural, =1 {# new reply} other {# new replies}}",
    values: {
      newReplies
    }
  }) : /*#__PURE__*/react.createElement(message/* default */.Z, {
    id: "threading.numReplies",
    defaultMessage: "{totalReplies, plural, =0 {Reply} =1 {# reply} other {# replies}}",
    values: {
      totalReplies
    }
  }))), showListTutorialTip && isFirstThreadInList && isMobileView && /*#__PURE__*/react.createElement(crt_list_tutorial_tip, {
    autoTour: tutorialTipAutoTour
  }));
}

ThreadItem.propTypes = {
  currentRelativeTeamUrl: (prop_types_default()).string.isRequired,
  displayName: (prop_types_default()).string.isRequired,
  postsInThread: (prop_types_default()).array.isRequired,
  isSelected: (prop_types_default()).bool.isRequired,
  style: (prop_types_default()).any,
  isFirstThreadInList: (prop_types_default()).bool.isRequired
};
/* harmony default export */ const thread_item = (/*#__PURE__*/(0,react.memo)(ThreadItem));
;// CONCATENATED MODULE: ./components/threading/global_threads/thread_item/index.ts
// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.










function makeMapStateToProps() {
  const getPostsForThread = (0,posts/* makeGetPostsForThread */.Zf)();
  const getChannel = (0,entities_channels/* makeGetChannel */.kL)();
  const getDisplayName = (0,users/* makeGetDisplayName */.RX)();
  return (state, ownProps) => {
    const {
      threadId
    } = ownProps;
    const post = (0,posts/* getPost */.xl)(state, threadId);
    return {
      post,
      channel: getChannel(state, {
        id: post.channel_id
      }),
      currentRelativeTeamUrl: (0,teams/* getCurrentRelativeTeamUrl */.gm)(state),
      displayName: getDisplayName(state, post.user_id, true),
      postsInThread: getPostsForThread(state, post.id),
      thread: (0,threads/* getThread */.bJ)(state, threadId)
    };
  };
}

/* harmony default export */ const global_threads_thread_item = ((0,redux.compose)((0,es.connect)(makeMapStateToProps), react.memo)(thread_item));
;// CONCATENATED MODULE: ./components/threading/global_threads/thread_list/virtualized_thread_list_row.tsx


function ownKeys(object, enumerableOnly) { var keys = Object.keys(object); if (Object.getOwnPropertySymbols) { var symbols = Object.getOwnPropertySymbols(object); if (enumerableOnly) { symbols = symbols.filter(function (sym) { return Object.getOwnPropertyDescriptor(object, sym).enumerable; }); } keys.push.apply(keys, symbols); } return keys; }

function _objectSpread(target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i] != null ? arguments[i] : {}; if (i % 2) { ownKeys(Object(source), true).forEach(function (key) { _defineProperty(target, key, source[key]); }); } else if (Object.getOwnPropertyDescriptors) { Object.defineProperties(target, Object.getOwnPropertyDescriptors(source)); } else { ownKeys(Object(source)).forEach(function (key) { Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key)); }); } } return target; }

function _defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }

// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.












function Row(_ref) {
  let {
    index,
    style,
    data
  } = _ref;
  const itemId = data.ids[index];
  const isSelected = data.selectedThreadId === itemId;

  if (itemId === constants/* Constants.THREADS_LOADING_INDICATOR_ITEM_ID */.gT.THREADS_LOADING_INDICATOR_ITEM_ID) {
    return /*#__PURE__*/react.createElement(loading_screen/* default */.Z, {
      message: /*#__PURE__*/react.createElement(react.Fragment, null),
      style: style
    });
  }

  if (itemId === constants/* Constants.THREADS_NO_RESULTS_ITEM_ID */.gT.THREADS_NO_RESULTS_ITEM_ID) {
    return /*#__PURE__*/react.createElement(no_results_indicator/* default */.Z, {
      style: _objectSpread(_objectSpread({}, style), {}, {
        padding: '16px 16px 16px 24px',
        background: 'rgba(var(--center-channel-color-rgb), 0.04)'
      }),
      iconGraphic: /*#__PURE__*/react.createElement(search_hint_svg, null),
      title: /*#__PURE__*/react.createElement(message/* default */.Z, {
        id: "globalThreads.searchGuidance.title",
        defaultMessage: "That\u2019s the end of the list"
      }),
      subtitle: /*#__PURE__*/react.createElement(message/* default */.Z, {
        id: "globalThreads.searchGuidance.subtitle",
        defaultMessage: "If you\u2019re looking for older conversations, try searching with {searchShortcut}",
        values: {
          searchShortcut: /*#__PURE__*/react.createElement(search_shortcut/* SearchShortcut */.B, {
            className: "thread-no-results-subtitle-shortcut",
            variant: shortcut_key/* ShortcutKeyVariant.TutorialTip */.S.TutorialTip
          })
        }
      }),
      titleClassName: "thread-no-results-title",
      subtitleClassName: "thread-no-results-subtitle",
      layout: types/* NoResultsLayout.Horizontal */.C.Horizontal
    });
  }

  return /*#__PURE__*/react.createElement(global_threads_thread_item, {
    isSelected: isSelected,
    key: itemId,
    style: style,
    threadId: itemId,
    isFirstThreadInList: index === 0
  });
}

Row.propTypes = {
  data: prop_types_default().shape({}).isRequired,
  index: (prop_types_default()).number.isRequired,
  style: (prop_types_default()).any.isRequired
};
/* harmony default export */ const virtualized_thread_list_row = (/*#__PURE__*/(0,react.memo)(Row, react_window_dist_index_esm/* areEqual */.wy));
;// CONCATENATED MODULE: ./components/threading/global_threads/thread_list/virtualized_thread_list.tsx


// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.






const style = {
  willChange: 'auto'
};

function VirtualizedThreadList(_ref) {
  let {
    ids,
    selectedThreadId,
    loadMoreItems,
    total,
    isLoading,
    addNoMoreResultsItem
  } = _ref;
  const infiniteLoaderRef = react.useRef();
  const startIndexRef = react.useRef(0);
  const stopIndexRef = react.useRef(0);
  const itemKey = (0,react.useCallback)(index => ids[index], [ids]);
  (0,react.useEffect)(() => {
    if (ids.length > 0 && selectedThreadId) {
      const index = ids.indexOf(selectedThreadId);

      if (startIndexRef.current >= index || index > stopIndexRef.current) {
        var _infiniteLoaderRef$cu;

        // eslint-disable-next-line no-underscore-dangle
        (_infiniteLoaderRef$cu = infiniteLoaderRef.current) === null || _infiniteLoaderRef$cu === void 0 ? void 0 : _infiniteLoaderRef$cu._listRef.scrollToItem(index);
      }
    } // ids should not be on the dependency list as
    // it will auto scroll to selected item upon
    // infinite loading
    // when the selectedThreadId changes it will get
    // the new ids so no issue there

  }, [selectedThreadId]);
  const data = (0,react.useMemo)(() => ({
    ids: addNoMoreResultsItem && ids.length === total ? [...ids, constants/* Constants.THREADS_NO_RESULTS_ITEM_ID */.gT.THREADS_NO_RESULTS_ITEM_ID] : isLoading && ids.length !== total && [...ids, constants/* Constants.THREADS_LOADING_INDICATOR_ITEM_ID */.gT.THREADS_LOADING_INDICATOR_ITEM_ID] || ids,
    selectedThreadId
  }), [ids, selectedThreadId, isLoading, addNoMoreResultsItem, total]);
  const isItemLoaded = (0,react.useCallback)(index => {
    return ids.length === total || index < ids.length;
  }, [ids, total]);
  return /*#__PURE__*/react.createElement(index_esm/* default */.Z, null, _ref2 => {
    let {
      height,
      width
    } = _ref2;
    return /*#__PURE__*/react.createElement(dist_index_esm/* default */.Z, {
      ref: infiniteLoaderRef,
      itemCount: total,
      loadMoreItems: loadMoreItems,
      isItemLoaded: isItemLoaded,
      minimumBatchSize: constants/* Constants.THREADS_PAGE_SIZE */.gT.THREADS_PAGE_SIZE
    }, _ref3 => {
      let {
        onItemsRendered: _onItemsRendered,
        ref
      } = _ref3;
      return /*#__PURE__*/react.createElement(react_window_dist_index_esm/* FixedSizeList */.t7, {
        onItemsRendered: _ref4 => {
          let {
            overscanStartIndex,
            overscanStopIndex,
            visibleStartIndex,
            visibleStopIndex
          } = _ref4;

          _onItemsRendered({
            overscanStartIndex,
            overscanStopIndex,
            visibleStartIndex,
            visibleStopIndex
          });

          startIndexRef.current = visibleStartIndex;
          stopIndexRef.current = visibleStopIndex;
        },
        ref: ref,
        height: height,
        itemCount: data.ids.length,
        itemData: data,
        itemKey: itemKey,
        itemSize: 133,
        style: style,
        width: width,
        className: "virtualized-thread-list"
      }, virtualized_thread_list_row);
    });
  });
}

VirtualizedThreadList.propTypes = {
  loadMoreItems: (prop_types_default()).func.isRequired,
  total: (prop_types_default()).number.isRequired,
  isLoading: (prop_types_default()).bool,
  addNoMoreResultsItem: (prop_types_default()).bool
};

function virtualized_thread_list_areEqual(prevProps, nextProps) {
  return prevProps.selectedThreadId === nextProps.selectedThreadId && prevProps.ids.join() === nextProps.ids.join() && prevProps.isLoading === nextProps.isLoading && prevProps.addNoMoreResultsItem === nextProps.addNoMoreResultsItem;
}

/* harmony default export */ const virtualized_thread_list = (/*#__PURE__*/(0,react.memo)(VirtualizedThreadList, virtualized_thread_list_areEqual));
;// CONCATENATED MODULE: ./components/threading/global_threads/thread_list/thread_list.tsx



// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.




















let ThreadFilter;

(function (ThreadFilter) {
  ThreadFilter["none"] = "";
  ThreadFilter["unread"] = "unread";
})(ThreadFilter || (ThreadFilter = {}));

const FILTER_STORAGE_KEY = 'globalThreads_filter';

const ThreadList = _ref => {
  let {
    currentFilter = ThreadFilter.none,
    someUnread,
    setFilter,
    selectedThreadId,
    unreadIds,
    ids
  } = _ref;
  const isMobileView = (0,es.useSelector)(browser/* getIsMobileView */.E7);
  const unread = ThreadFilter.unread === currentFilter;
  const data = unread ? unreadIds : ids;
  const ref = react.useRef(null);
  const {
    currentTeamId,
    currentUserId,
    clear,
    select
  } = (0,hooks/* useThreadRouting */.Q)();
  const tipStep = (0,es.useSelector)(state => (0,preferences/* getInt */.wS)(state, constants/* Preferences.CRT_TUTORIAL_STEP */.uP.CRT_TUTORIAL_STEP, currentUserId));
  const showListTutorialTip = tipStep === constants/* CrtTutorialSteps.LIST_POPOVER */.WC.LIST_POPOVER;
  const showUnreadTutorialTip = tipStep === constants/* CrtTutorialSteps.UNREAD_POPOVER */.WC.UNREAD_POPOVER;
  const tutorialTipAutoTour = (0,es.useSelector)(state => (0,preferences/* getInt */.wS)(state, constants/* Preferences.CRT_TUTORIAL_AUTO_TOUR_STATUS */.uP.CRT_TUTORIAL_AUTO_TOUR_STATUS, currentUserId, constants/* Constants.AutoTourStatus.ENABLED */.gT.AutoTourStatus.ENABLED)) === constants/* Constants.AutoTourStatus.ENABLED */.gT.AutoTourStatus.ENABLED;
  const {
    formatMessage
  } = (0,useIntl/* default */.Z)();
  const dispatch = (0,es.useDispatch)();
  const {
    total = 0,
    total_unread_threads: totalUnread
  } = (0,es.useSelector)(threads/* getThreadCountsInCurrentTeam */.Pd);
  const [isLoading, setLoading] = react.useState(false);
  const [hasLoaded, setHasLoaded] = react.useState(false);
  const handleKeyDown = (0,react.useCallback)(e => {
    var _target$tagName, _ref$current;

    // Ensure that arrow keys navigation is not triggered if the textbox is focused
    const target = e.target;
    const tagName = target === null || target === void 0 ? void 0 : (_target$tagName = target.tagName) === null || _target$tagName === void 0 ? void 0 : _target$tagName.toLowerCase();

    if (tagName === 'input' || tagName === 'textarea' || tagName === 'select') {
      return;
    }

    const comboKeyPressed = e.altKey || e.metaKey || e.shiftKey || e.ctrlKey;

    if (comboKeyPressed || !utils/* isKeyPressed */.pD(e, constants/* Constants.KeyCodes.DOWN */.gT.KeyCodes.DOWN) && !utils/* isKeyPressed */.pD(e, constants/* Constants.KeyCodes.UP */.gT.KeyCodes.UP)) {
      return;
    }

    let threadIdToSelect = 0;

    if (selectedThreadId) {
      const selectedThreadIndex = data.indexOf(selectedThreadId);

      if (utils/* isKeyPressed */.pD(e, constants/* Constants.KeyCodes.DOWN */.gT.KeyCodes.DOWN)) {
        if (selectedThreadIndex < data.length - 1) {
          threadIdToSelect = selectedThreadIndex + 1;
        }

        if (selectedThreadIndex === data.length - 1) {
          return;
        }
      }

      if (utils/* isKeyPressed */.pD(e, constants/* Constants.KeyCodes.UP */.gT.KeyCodes.UP)) {
        if (selectedThreadIndex > 0) {
          threadIdToSelect = selectedThreadIndex - 1;
        } else {
          return;
        }
      }
    }

    select(data[threadIdToSelect]); // hacky way to ensure the thread item loses focus.

    (_ref$current = ref.current) === null || _ref$current === void 0 ? void 0 : _ref$current.focus();
  }, [selectedThreadId, data]);
  (0,react.useEffect)(() => {
    document.addEventListener('keydown', handleKeyDown);
    return () => {
      document.removeEventListener('keydown', handleKeyDown);
    };
  }, [handleKeyDown]);
  const handleRead = (0,react.useCallback)(() => {
    setFilter(ThreadFilter.none);
  }, [setFilter]);
  const handleUnread = (0,react.useCallback)(() => {
    (0,telemetry_actions/* trackEvent */.L9)('crt', 'filter_threads_by_unread');
    setFilter(ThreadFilter.unread);
  }, [setFilter]);
  const handleLoadMoreItems = (0,react.useCallback)(async startIndex => {
    setLoading(true);
    let before = data[startIndex - 1];

    if (before === selectedThreadId) {
      before = data[startIndex - 2];
    }

    await dispatch((0,actions_threads/* getThreads */.hr)(currentUserId, currentTeamId, {
      unread,
      perPage: constants/* Constants.THREADS_PAGE_SIZE */.gT.THREADS_PAGE_SIZE,
      before
    }));
    setLoading(false);
    setHasLoaded(true);
    return {
      data: true
    };
  }, [currentTeamId, data, unread, selectedThreadId]);
  const handleAllMarkedRead = (0,react.useCallback)(() => {
    (0,telemetry_actions/* trackEvent */.L9)('crt', 'mark_all_threads_read');
    dispatch((0,actions_threads/* markAllThreadsInTeamRead */.lL)(currentUserId, currentTeamId));

    if (currentFilter === ThreadFilter.unread) {
      clear();
    }
  }, [currentTeamId, currentUserId, currentFilter]);
  return /*#__PURE__*/react.createElement("div", {
    tabIndex: 0,
    ref: ref,
    className: 'ThreadList',
    id: 'threads-list-container'
  }, /*#__PURE__*/react.createElement(header, {
    id: 'tutorial-threads-mobile-header',
    heading: /*#__PURE__*/react.createElement(react.Fragment, null, /*#__PURE__*/react.createElement("div", {
      className: 'tab-button-wrapper'
    }, /*#__PURE__*/react.createElement(common_button/* default */.Z, {
      className: 'Button___large Margined',
      isActive: currentFilter === ThreadFilter.none,
      onClick: handleRead
    }, /*#__PURE__*/react.createElement(message/* default */.Z, {
      id: "threading.filters.allThreads",
      defaultMessage: "All your threads"
    }))), /*#__PURE__*/react.createElement("div", {
      id: 'threads-list-unread-button',
      className: 'tab-button-wrapper'
    }, /*#__PURE__*/react.createElement(common_button/* default */.Z, {
      className: 'Button___large Margined',
      isActive: currentFilter === ThreadFilter.unread,
      hasDot: someUnread,
      onClick: handleUnread
    }, /*#__PURE__*/react.createElement(message/* default */.Z, {
      id: "threading.filters.unreads",
      defaultMessage: "Unreads"
    })), showUnreadTutorialTip && /*#__PURE__*/react.createElement(crt_unread_tutorial_tip, {
      autoTour: tutorialTipAutoTour
    }))),
    right: /*#__PURE__*/react.createElement("div", {
      className: "right-anchor"
    }, /*#__PURE__*/react.createElement(simple_tooltip/* default */.Z, {
      id: "threadListMarkRead",
      content: formatMessage({
        id: 'threading.threadList.markRead',
        defaultMessage: 'Mark all as read'
      })
    }, /*#__PURE__*/react.createElement(common_button/* default */.Z, {
      id: 'threads-list__mark-all-as-read',
      className: 'Button___large Button___icon',
      onClick: handleAllMarkedRead
    }, /*#__PURE__*/react.createElement("span", {
      className: "Icon"
    }, /*#__PURE__*/react.createElement("i", {
      className: "icon-playlist-check"
    })))))
  }), /*#__PURE__*/react.createElement("div", {
    className: "threads",
    "data-testid": 'threads_list'
  }, /*#__PURE__*/react.createElement(virtualized_thread_list, {
    key: "threads_list_".concat(currentFilter),
    loadMoreItems: handleLoadMoreItems,
    ids: data,
    selectedThreadId: selectedThreadId,
    total: unread ? totalUnread : total,
    isLoading: isLoading,
    addNoMoreResultsItem: hasLoaded && !unread
  }), showListTutorialTip && !isMobileView && /*#__PURE__*/react.createElement(crt_list_tutorial_tip, {
    autoTour: tutorialTipAutoTour
  }), unread && !someUnread && isEmpty_default()(unreadIds) ? /*#__PURE__*/react.createElement(no_results_indicator/* default */.Z, {
    expanded: true,
    iconGraphic: balloon_illustration,
    title: formatMessage({
      id: 'globalThreads.threadList.noUnreadThreads',
      defaultMessage: 'No unread threads'
    })
  }) : null));
};

/* harmony default export */ const thread_list = (/*#__PURE__*/(0,react.memo)(ThreadList));
;// CONCATENATED MODULE: ./components/threading/global_threads/thread_list/index.ts
// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.

/* eslint-disable no-duplicate-imports */


// EXTERNAL MODULE: ./components/threading/common/follow_button/index.ts + 1 modules
var follow_button = __webpack_require__(61577);
;// CONCATENATED MODULE: ./components/threading/global_threads/thread_pane/thread_pane.tsx

// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.














const getChannel = (0,entities_channels/* makeGetChannel */.kL)();
const getPostsForThread = (0,posts/* makeGetPostsForThread */.Zf)();

const ThreadPane = _ref => {
  let {
    thread,
    children
  } = _ref;
  const {
    formatMessage
  } = (0,useIntl/* default */.Z)();
  const dispatch = (0,es.useDispatch)();
  const {
    currentTeamId,
    currentUserId,
    goToInChannel,
    select
  } = (0,hooks/* useThreadRouting */.Q)();
  const {
    id: threadId,
    is_following: isFollowing,
    post: {
      channel_id: channelId
    }
  } = thread;
  const channel = (0,es.useSelector)(state => getChannel(state, {
    id: channelId
  }));
  const post = (0,es.useSelector)(state => (0,posts/* getPost */.xl)(state, thread.id));
  const postsInThread = (0,es.useSelector)(state => getPostsForThread(state, post.id));
  const selectHandler = (0,react.useCallback)(() => select(), []);
  let unreadTimestamp = post.edit_at || post.create_at; // if we have the whole thread, get the posts in it, sorted from newest to oldest.
  // Last post - root post, second to last post - oldest reply. Use that timestamp

  if (postsInThread.length > 1) {
    const p = postsInThread[postsInThread.length - 2];
    unreadTimestamp = p.edit_at || p.create_at;
  }

  const goToInChannelHandler = (0,react.useCallback)(() => {
    goToInChannel(threadId);
  }, [goToInChannel, threadId]);
  const followHandler = (0,react.useCallback)(() => {
    dispatch((0,actions_threads/* setThreadFollow */.NC)(currentUserId, currentTeamId, threadId, !isFollowing));
  }, [currentUserId, currentTeamId, threadId, isFollowing, actions_threads/* setThreadFollow */.NC]);
  return /*#__PURE__*/react.createElement("div", {
    id: 'thread-pane-container',
    className: "ThreadPane"
  }, /*#__PURE__*/react.createElement(header, {
    className: "ThreadPane___header",
    heading: /*#__PURE__*/react.createElement(react.Fragment, null, /*#__PURE__*/react.createElement(common_button/* default */.Z, {
      className: "Button___icon Button___large back",
      onClick: selectHandler
    }, /*#__PURE__*/react.createElement("i", {
      className: "icon icon-arrow-back-ios"
    })), /*#__PURE__*/react.createElement("h3", null, /*#__PURE__*/react.createElement("span", {
      className: "separated"
    }, formatMessage({
      id: 'threading.header.heading',
      defaultMessage: 'Thread'
    })), /*#__PURE__*/react.createElement(common_button/* default */.Z, {
      className: "separated",
      allowTextOverflow: true,
      onClick: goToInChannelHandler
    }, channel === null || channel === void 0 ? void 0 : channel.display_name))),
    right: /*#__PURE__*/react.createElement(react.Fragment, null, /*#__PURE__*/react.createElement(follow_button/* default */.Z, {
      isFollowing: isFollowing,
      disabled: isFollowing == null,
      onClick: followHandler
    }), /*#__PURE__*/react.createElement(thread_menu, {
      threadId: threadId,
      isFollowing: isFollowing,
      hasUnreads: Boolean(thread.unread_replies || thread.unread_mentions),
      unreadTimestamp: unreadTimestamp
    }, /*#__PURE__*/react.createElement(simple_tooltip/* default */.Z, {
      id: "threadActionMenu",
      content: formatMessage({
        id: (0,i18n.t)('threading.threadHeader.menu'),
        defaultMessage: 'More Actions'
      })
    }, /*#__PURE__*/react.createElement(common_button/* default */.Z, {
      className: "Button___icon Button___large"
    }, /*#__PURE__*/react.createElement("i", {
      className: "Icon icon-dots-vertical"
    })))))
  }), children);
};

ThreadPane.propTypes = {
  children: (prop_types_default()).node
};
/* harmony default export */ const thread_pane = (/*#__PURE__*/(0,react.memo)(ThreadPane));
;// CONCATENATED MODULE: ./components/threading/global_threads/thread_pane/index.ts
// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.

;// CONCATENATED MODULE: ./components/threading/global_threads/global_threads.tsx



// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.



























const GlobalThreads = () => {
  const {
    formatMessage
  } = (0,useIntl/* default */.Z)();
  const dispatch = (0,es.useDispatch)();
  const {
    url,
    params: {
      threadIdentifier
    }
  } = (0,react_router/* useRouteMatch */.$B)();
  const [filter, setFilter] = useGlobalState(ThreadFilter.none, FILTER_STORAGE_KEY);
  const {
    currentTeamId,
    currentUserId,
    clear
  } = (0,hooks/* useThreadRouting */.Q)();
  const counts = (0,es.useSelector)(threads/* getThreadCountsInCurrentTeam */.Pd);
  const selectedThread = (0,es.useSelector)(state => (0,threads/* getThread */.bJ)(state, threadIdentifier));
  const selectedThreadId = (0,es.useSelector)(selectors_views_threads/* getSelectedThreadIdInCurrentTeam */.Qk);
  const selectedPost = (0,es.useSelector)(state => (0,posts/* getPost */.xl)(state, threadIdentifier));
  const threadIds = (0,es.useSelector)(state => (0,threads/* getThreadOrderInCurrentTeam */.Jf)(state, selectedThread === null || selectedThread === void 0 ? void 0 : selectedThread.id), es.shallowEqual);
  const unreadThreadIds = (0,es.useSelector)(state => (0,threads/* getUnreadThreadOrderInCurrentTeam */.pm)(state, selectedThread === null || selectedThread === void 0 ? void 0 : selectedThread.id), es.shallowEqual);
  const numUnread = (counts === null || counts === void 0 ? void 0 : counts.total_unread_threads) || 0;
  (0,react.useEffect)(() => {
    dispatch(rhs/* suppressRHS */.TT);
    dispatch((0,channels/* selectChannel */.QE)(''));
    dispatch(global_actions/* clearLastUnreadChannel */.Rn);
    (0,user_actions/* loadProfilesForSidebar */.D3)();
    const penultimateType = local_storage_store/* default.getPreviousViewedType */.Z.getPreviousViewedType(currentUserId, currentTeamId);

    if (penultimateType !== constants/* PreviousViewedTypes.THREADS */.IY.THREADS) {
      local_storage_store/* default.setPenultimateViewedType */.Z.setPenultimateViewedType(currentUserId, currentTeamId, penultimateType);
      local_storage_store/* default.setPreviousViewedType */.Z.setPreviousViewedType(currentUserId, currentTeamId, constants/* PreviousViewedTypes.THREADS */.IY.THREADS);
    } // unsuppresses RHS on navigating away (unmount)


    return () => {
      dispatch(rhs/* unsuppressRHS */.ht);
    };
  }, []);
  (0,react.useEffect)(() => {
    dispatch((0,actions_threads/* getThreadCounts */.qN)(currentUserId, currentTeamId));
  }, [currentTeamId, currentUserId]);
  (0,react.useEffect)(() => {
    if (!selectedThreadId || selectedThreadId !== threadIdentifier) {
      dispatch((0,views_threads/* setSelectedThreadId */.c9)(currentTeamId, selectedThread === null || selectedThread === void 0 ? void 0 : selectedThread.id));
    }
  }, [currentTeamId, selectedThreadId, threadIdentifier]);

  const isEmptyList = isEmpty_default()(threadIds) && isEmpty_default()(unreadThreadIds);

  const [isLoading, setLoading] = (0,react.useState)(isEmptyList);
  const fetchThreads = (0,react.useCallback)(async unread => {
    await dispatch((0,actions_threads/* getThreads */.hr)(currentUserId, currentTeamId, {
      unread,
      perPage: constants/* Constants.THREADS_PAGE_SIZE */.gT.THREADS_PAGE_SIZE
    }));
    return {
      data: true
    };
  }, [currentUserId, currentTeamId]);

  const isOnlySelectedThreadInList = list => {
    return selectedThreadId && list.length === 1 && list[0] === selectedThreadId;
  };

  const shouldLoadThreads = isEmpty_default()(threadIds) || isOnlySelectedThreadInList(threadIds);
  const shouldLoadUnreadThreads = isEmpty_default()(unreadThreadIds) || isOnlySelectedThreadInList(unreadThreadIds);
  (0,react.useEffect)(() => {
    const promises = []; // this is needed to jump start threads fetching

    if (shouldLoadThreads) {
      promises.push(fetchThreads(false));
    }

    if (filter === ThreadFilter.unread && shouldLoadUnreadThreads) {
      promises.push(fetchThreads(true));
    }

    Promise.all(promises).then(() => {
      setLoading(false);
    });
  }, [fetchThreads, filter, threadIds, unreadThreadIds]);
  (0,react.useEffect)(() => {
    if (!selectedThread && !selectedPost && !isLoading) {
      clear();
    }
  }, [currentTeamId, selectedThread, selectedPost, isLoading, counts, filter]); // cleanup on unmount

  (0,react.useEffect)(() => {
    return () => {
      dispatch((0,views_threads/* setSelectedThreadId */.c9)(currentTeamId, ''));
    };
  }, []);
  const handleSelectUnread = (0,react.useCallback)(() => {
    setFilter(ThreadFilter.unread);
  }, []);
  return /*#__PURE__*/react.createElement("div", {
    id: "app-content",
    className: classnames_default()('GlobalThreads app__content', {
      'thread-selected': Boolean(selectedThread)
    })
  }, /*#__PURE__*/react.createElement(header, {
    level: 2,
    className: 'GlobalThreads___header',
    heading: formatMessage({
      id: 'globalThreads.heading',
      defaultMessage: 'Followed threads'
    }),
    subtitle: formatMessage({
      id: 'globalThreads.subtitle',
      defaultMessage: 'Threads you’re participating in will automatically show here'
    })
  }), isLoading || isEmptyList ? /*#__PURE__*/react.createElement("div", {
    className: "no-results__holder"
  }, isLoading ? /*#__PURE__*/react.createElement(loading_screen/* default */.Z, null) : /*#__PURE__*/react.createElement(no_results_indicator/* default */.Z, {
    expanded: true,
    iconGraphic: chat_illustration,
    title: formatMessage({
      id: 'globalThreads.noThreads.title',
      defaultMessage: 'No followed threads yet'
    }),
    subtitle: formatMessage({
      id: 'globalThreads.noThreads.subtitle',
      defaultMessage: 'Any threads you are mentioned in or have participated in will show here along with any threads you have followed.'
    })
  })) : /*#__PURE__*/react.createElement(react.Fragment, null, /*#__PURE__*/react.createElement(thread_list, {
    currentFilter: filter,
    setFilter: setFilter,
    someUnread: Boolean(numUnread),
    selectedThreadId: threadIdentifier,
    ids: threadIds,
    unreadIds: unreadThreadIds
  }), selectedThread && selectedPost ? /*#__PURE__*/react.createElement(thread_pane, {
    thread: selectedThread
  }, /*#__PURE__*/react.createElement(thread_viewer/* default */.Z, {
    rootPostId: selectedThread.id,
    useRelativeTimestamp: true,
    isThreadView: true
  })) : /*#__PURE__*/react.createElement(no_results_indicator/* default */.Z, {
    expanded: true,
    iconGraphic: chat_illustration,
    title: formatMessage({
      id: 'globalThreads.threadPane.unselectedTitle',
      defaultMessage: '{numUnread, plural, =0 {Looks like you’re all caught up} other {Catch up on your threads}}'
    }, {
      numUnread
    }),
    subtitle: formatMessage({
      id: 'globalThreads.threadPane.unreadMessageLink',
      defaultMessage: 'You have {numUnread, plural, =0 {no unread threads} =1 {<link>{numUnread} thread</link>} other {<link>{numUnread} threads</link>}} {numUnread, plural, =0 {} other {with unread messages}}'
    }, {
      numUnread,
      link: chunks => /*#__PURE__*/react.createElement(react_router_dom.Link, {
        key: "single",
        to: "".concat(url, "/").concat(unreadThreadIds[0]),
        onClick: handleSelectUnread
      }, chunks)
    })
  })));
};

/* harmony default export */ const global_threads = (/*#__PURE__*/(0,react.memo)(GlobalThreads));
;// CONCATENATED MODULE: ./components/threading/global_threads/index.ts
// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.


/***/ }),

/***/ 86535:
/***/ ((__unused_webpack_module, __unused_webpack_exports, __webpack_require__) => {

"use strict";

var $ = __webpack_require__(82109);
var flattenIntoArray = __webpack_require__(6790);
var aCallable = __webpack_require__(19662);
var toObject = __webpack_require__(47908);
var lengthOfArrayLike = __webpack_require__(26244);
var arraySpeciesCreate = __webpack_require__(65417);

// `Array.prototype.flatMap` method
// https://tc39.es/ecma262/#sec-array.prototype.flatmap
$({ target: 'Array', proto: true }, {
  flatMap: function flatMap(callbackfn /* , thisArg */) {
    var O = toObject(this);
    var sourceLen = lengthOfArrayLike(O);
    var A;
    aCallable(callbackfn);
    A = arraySpeciesCreate(O, 0);
    A.length = flattenIntoArray(A, O, O, sourceLen, 0, 1, callbackfn, arguments.length > 1 ? arguments[1] : undefined);
    return A;
  }
});


/***/ }),

/***/ 86587:
/***/ ((__unused_webpack_module, __unused_webpack_exports, __webpack_require__) => {

// this method was added to unscopables after implementation
// in popular engines, so it's moved to a separate module
var addToUnscopables = __webpack_require__(51223);

// https://tc39.es/ecma262/#sec-array.prototype-@@unscopables
addToUnscopables('flatMap');


/***/ })

}]);
//# sourceMappingURL=31.5620fac86c98f0a265b6.js.map