"use strict";
(self["webpackChunk_mattermost_webapp"] = self["webpackChunk_mattermost_webapp"] || []).push([[741],{

/***/ 71284:
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "Z": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
/* harmony import */ var prop_types__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(45697);
/* harmony import */ var prop_types__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(prop_types__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(67294);
/* harmony import */ var utils_constants__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(66788);

// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.




const Column = _ref => {
  let {
    title,
    message,
    SVGElement,
    extraContent,
    onEnterKeyDown
  } = _ref;

  const handleOnEnterKeyDown = e => {
    if (onEnterKeyDown && e.key === utils_constants__WEBPACK_IMPORTED_MODULE_1__/* ["default"].KeyCodes.ENTER[0] */ .ZP.KeyCodes.ENTER[0]) {
      onEnterKeyDown(e);
    }
  };

  return /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0__.createElement("div", {
    className: "content-layout-column",
    onKeyDown: handleOnEnterKeyDown,
    tabIndex: 0
  }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0__.createElement("div", {
    className: "content-layout-column-svg"
  }, SVGElement), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0__.createElement("h1", {
    className: "content-layout-column-title"
  }, title), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0__.createElement("p", {
    className: "content-layout-column-message"
  }, message), extraContent && /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0__.createElement("div", {
    className: "content-layout-column-extra-content"
  }, extraContent));
};

Column.propTypes = {
  title: (prop_types__WEBPACK_IMPORTED_MODULE_2___default().string.isRequired),
  message: (prop_types__WEBPACK_IMPORTED_MODULE_2___default().string.isRequired),
  SVGElement: (prop_types__WEBPACK_IMPORTED_MODULE_2___default().node),
  extraContent: (prop_types__WEBPACK_IMPORTED_MODULE_2___default().node),
  onEnterKeyDown: (prop_types__WEBPACK_IMPORTED_MODULE_2___default().func)
};
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (Column);

/***/ }),

/***/ 94741:
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

// ESM COMPAT FLAG
__webpack_require__.r(__webpack_exports__);

// EXPORTS
__webpack_require__.d(__webpack_exports__, {
  "default": () => (/* binding */ should_verify_email)
});

// EXTERNAL MODULE: ./node_modules/core-js/modules/es.regexp.exec.js
var es_regexp_exec = __webpack_require__(74916);
// EXTERNAL MODULE: ./node_modules/core-js/modules/es.string.search.js
var es_string_search = __webpack_require__(64765);
// EXTERNAL MODULE: ./node_modules/core-js/modules/web.dom-collections.iterator.js
var web_dom_collections_iterator = __webpack_require__(33948);
// EXTERNAL MODULE: ./node_modules/core-js/modules/web.url-search-params.js
var web_url_search_params = __webpack_require__(41637);
// EXTERNAL MODULE: ./node_modules/core-js/modules/es.promise.js
var es_promise = __webpack_require__(88674);
// EXTERNAL MODULE: ./node_modules/react/index.js
var react = __webpack_require__(67294);
// EXTERNAL MODULE: ./node_modules/react-intl/lib/src/components/useIntl.js
var useIntl = __webpack_require__(86896);
// EXTERNAL MODULE: ./node_modules/react-redux/es/index.js + 19 modules
var es = __webpack_require__(14494);
// EXTERNAL MODULE: ./node_modules/react-router/esm/react-router.js + 1 modules
var react_router = __webpack_require__(5977);
// EXTERNAL MODULE: ./node_modules/classnames/index.js
var classnames = __webpack_require__(94184);
var classnames_default = /*#__PURE__*/__webpack_require__.n(classnames);
// EXTERNAL MODULE: ./node_modules/prop-types/index.js
var prop_types = __webpack_require__(45697);
var prop_types_default = /*#__PURE__*/__webpack_require__.n(prop_types);
// EXTERNAL MODULE: ./node_modules/core-js/modules/es.regexp.to-string.js
var es_regexp_to_string = __webpack_require__(39714);
;// CONCATENATED MODULE: ./components/common/svg_images_components/man_with_mailbox_svg.tsx


// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.

/* eslint-disable max-lines */


const Svg = props => {
  var _props$width, _props$height;

  return /*#__PURE__*/react.createElement("svg", {
    width: ((_props$width = props.width) === null || _props$width === void 0 ? void 0 : _props$width.toString()) || '308',
    height: ((_props$height = props.height) === null || _props$height === void 0 ? void 0 : _props$height.toString()) || '223',
    viewBox: "0 0 308 223",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    xmlnsXlink: "http://www.w3.org/1999/xlink"
  }, /*#__PURE__*/react.createElement("path", {
    d: "M217.912 130.588V187.401C217.682 189.441 215.652 191.142 211.821 192.502C207.989 193.862 203.144 194.545 197.285 194.551C191.427 194.551 186.581 193.868 182.75 192.502C178.919 191.136 176.891 189.436 176.668 187.401V130.588H217.912Z",
    fill: "var(--away-indicator)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M279.094 150.362C279.083 150.701 279.002 151.034 278.858 151.341C278.714 151.648 278.508 151.922 278.255 152.145C278.001 152.369 277.705 152.538 277.383 152.642C277.062 152.745 276.724 152.782 276.388 152.748H189.176L151.317 45.5386H241.244C251.839 45.9939 260.684 49.7953 267.78 56.9426C274.876 64.0899 278.65 72.9923 279.103 83.6499L279.094 150.362Z",
    fill: "#BABEC9"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M178.188 56.9338C185.284 64.0753 188.947 72.9778 189.176 83.6411V150.362C188.952 151.953 188.162 152.748 186.813 152.748H116.155C115.834 152.793 115.507 152.762 115.199 152.658C114.892 152.553 114.612 152.379 114.383 152.147C114.154 151.916 113.982 151.634 113.879 151.325C113.777 151.015 113.747 150.685 113.793 150.362V83.6411C114.016 72.9778 117.679 64.0753 124.78 56.9338C131.882 49.7922 140.727 45.9909 151.317 45.5298C162.135 45.9851 171.092 49.7865 178.188 56.9338Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M282.135 90.2724C283.951 92.6838 285.01 95.586 285.176 98.6071V118.683C285.061 122.288 283.543 125.704 280.95 128.193C279.671 129.431 278.16 130.401 276.505 131.045C274.85 131.689 273.085 131.996 271.311 131.946C269.565 132.004 267.827 131.701 266.201 131.058C264.576 130.414 263.099 129.442 261.861 128.202C259.36 125.664 257.911 122.265 257.806 118.692V100.648H216.228C215.201 100.682 214.178 100.505 213.222 100.127C212.266 99.7484 211.398 99.1772 210.669 98.4482C209.941 97.7193 209.369 96.8479 208.987 95.8879C208.606 94.9279 208.425 93.8996 208.453 92.8662C208.406 90.7227 209.195 88.6459 210.652 87.082C211.358 86.3061 212.221 85.6921 213.183 85.2818C214.145 84.8715 215.183 84.6746 216.228 84.7044H271.311C272.473 84.6656 273.628 84.8963 274.687 85.3788C277.64 86.1341 280.26 87.8555 282.135 90.2724Z",
    fill: "var(--dnd-indicator)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M189.176 112.899L186.47 114.914C183.761 116.954 181.396 117.975 179.374 117.975C177.352 117.975 174.984 116.954 172.269 114.914L149.324 97.6221C136.249 87.4084 128.804 81.2842 126.988 79.2495C127.594 77.2995 128.846 75.6179 130.536 74.4855C132.178 73.356 134.122 72.7531 136.111 72.7564H187.492C188.645 76.2706 189.225 79.9495 189.21 83.6503L189.176 112.899Z",
    fill: "#DDDFE4"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M189.176 106.769L179.391 99.2905L126.301 139.122V82.6211C126.477 80.01 127.552 77.5424 129.342 75.6439C130.253 74.6816 131.356 73.9255 132.58 73.4261C133.803 72.9267 135.119 72.6953 136.438 72.7474H187.484C188.636 76.2617 189.217 79.9406 189.202 83.6414L189.176 106.769Z",
    fill: "#BABEC9"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M189.176 146.238H135.768L134.41 145.901C132.407 145.687 130.519 144.853 129.007 143.514L127.993 142.494C127.123 141.675 126.523 140.607 126.275 139.434C127.844 137.393 135.393 131.269 148.92 121.061L171.909 103.371C174.618 101.106 177.04 99.9706 179.176 99.9649C181.312 99.9591 183.735 101.095 186.444 103.371L189.15 105.412L189.176 146.238Z",
    fill: "#989DAE"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M189.176 112.899L186.47 114.914C183.761 116.954 181.396 117.974 179.374 117.974C177.352 117.974 174.984 116.954 172.269 114.914L126.988 79.2148C126.988 77.1484 129.686 75.1338 129.686 75.1338C131.204 73.7995 133.093 72.9665 135.098 72.7476H187.501C188.653 76.2618 189.234 79.9407 189.219 83.6415L189.176 112.899Z",
    fill: "#DDDFE4"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M58.3277 52.8529L58.7053 41.5931L57.8473 37.134C59.4174 33.0794 61.7253 33.3635 63.1409 33.2344C66.3668 32.9417 68.5203 31.1684 70.0046 32.7351C70.8111 33.6734 71.7205 37.5041 71.8664 40.4826C71.9693 42.058 72.5184 43.4009 71.3516 44.1929C70.5473 44.633 69.7061 45.0017 68.8378 45.2948L69.2668 53.7224L58.3277 52.8529Z",
    fill: "var(--away-indicator)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M70.8711 36.9189C71.9397 37.6604 72.9161 38.5274 73.7796 39.5015C74.0455 40.6808 70.811 41.2834 70.811 41.2834L70.8711 36.9189Z",
    fill: "var(--away-indicator)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M65.4058 45.7424C66.55 45.8676 67.7075 45.7232 68.7862 45.3206C69.1895 45.157 68.0398 45.8887 67.628 46.0351C67.0986 46.3077 66.506 46.4325 65.912 46.3966C65.5002 46.3277 64.9683 45.7079 65.4058 45.7424Z",
    fill: "#CC8F00"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M74.3544 31.3921C74.2163 31.0541 73.9586 30.7792 73.6308 30.6203C73.303 30.4614 72.9281 30.4297 72.5784 30.5313C69.5327 31.0392 70.1075 28.3792 65.6203 29.1368C62.4974 29.6619 59.2028 29.1367 56.4402 30.7293C55.2623 31.3031 54.3084 32.2542 53.729 33.4323C53.4583 34.0245 53.4028 34.6931 53.5721 35.322C53.7413 35.951 54.1247 36.5007 54.6556 36.8757C54.8443 36.9876 55.0674 37.0823 55.1618 37.2803C55.2222 37.4861 55.2039 37.7071 55.1103 37.9001C54.7242 39.0622 54.3811 40.4826 55.2476 41.3435C55.7281 41.8255 56.5345 42.0494 56.7233 42.7036C56.8262 43.0651 56.7233 43.4525 56.7662 43.8313C56.9463 44.8299 58.4821 44.9934 59.2114 44.2875C59.8578 43.4856 60.1957 42.4776 60.1637 41.4467C60.2409 40.5429 60.2667 39.5013 59.5975 38.8642C59.2886 38.5715 58.851 38.3994 58.688 38.0034C58.6225 37.7639 58.6346 37.5098 58.7225 37.2777C58.8104 37.0456 58.9696 36.8475 59.1771 36.7121C59.4551 36.5156 59.7718 36.3812 60.106 36.3181C60.4402 36.2549 60.784 36.2645 61.1142 36.3461C61.4444 36.4277 61.7533 36.5794 62.0201 36.7911C62.2868 37.0028 62.5052 37.2694 62.6604 37.573C62.9092 38.1411 62.9864 38.8901 63.5183 39.1656C64.2133 39.5099 65.0026 38.7351 65.097 37.969C65.0874 37.1946 65.0099 36.4226 64.8653 35.6619C64.8653 34.8872 65.26 33.9403 66.0322 33.9403C66.2874 33.9535 66.5391 34.0058 66.7786 34.0952C68.2943 34.524 69.893 34.5655 71.4287 34.2157C72.1912 34.104 72.9154 33.8087 73.5393 33.3549C73.8404 33.1332 74.0755 32.8332 74.2192 32.4873C74.3628 32.1414 74.4096 31.7628 74.3544 31.3921Z",
    fill: "#66320A"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M68.4687 36.1957L68.5116 36.1096C66.2209 35.903 63.3638 35.7222 61.4077 35.7481C59.4515 35.7739 58.5507 36.0494 57.5297 37.8141C57.3581 38.1154 57.8557 38.3306 58.0359 38.0293C58.2388 37.546 58.5718 37.1289 58.9976 36.8249C59.4234 36.5208 59.9252 36.3417 60.4468 36.3076C61.442 36.2302 62.4716 36.3076 63.4582 36.3076C64.7194 36.3076 66.6756 36.4884 68.3057 36.6433C68.3572 36.4884 68.4087 36.3421 68.4687 36.1957Z",
    fill: "var(--center-channel-bg)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M68.7692 37.8742C68.5157 37.6872 68.3205 37.4316 68.2066 37.1374C68.0926 36.8431 68.0645 36.5224 68.1257 36.2127C68.2458 35.3088 68.915 34.6547 69.61 34.7493C69.9833 34.8561 70.3005 35.1046 70.4943 35.4421C70.6881 35.7795 70.7433 36.1795 70.6481 36.5571C70.528 37.4179 69.8588 38.1152 69.1638 38.0205C69.0231 38.0016 68.8883 37.9516 68.7692 37.8742ZM69.7987 35.2917C69.7273 35.2429 69.6446 35.2132 69.5585 35.2056C69.0866 35.1367 68.6319 35.6274 68.5375 36.2988C68.4431 36.9703 68.752 37.5729 69.2239 37.6418C69.6957 37.7106 70.1419 37.2113 70.2363 36.5399C70.2943 36.3065 70.2838 36.0613 70.2062 35.8337C70.1285 35.6062 69.987 35.406 69.7987 35.2572V35.2917Z",
    fill: "var(--center-channel-bg)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M64.5479 52.121C86.8549 52.2932 86.7948 50.1927 89.0341 76.5947C91.2734 102.997 91.3077 103.952 69.2495 105.407C54.5441 106.38 45.8615 112.294 45.0979 104.185C44.3258 95.9205 43.8367 96.1615 42.524 82.1213C39.5469 51.0536 40.0531 51.9316 64.5479 52.121Z",
    fill: "var(--away-indicator)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M74.5689 71.6366C80.935 78.8849 95.3144 84.4889 106.425 84.4889C108.012 84.4988 109.593 84.2961 111.127 83.8863C112.069 83.631 112.985 83.288 113.864 82.8619C123.009 78.3769 125.729 64.5346 127.72 59.7828C130.173 53.9377 127.625 57.5705 129.281 54.2304C129.873 53.0339 131.332 52.2591 131.804 51.6479C132.481 50.7871 131.186 49.6249 126.295 52.9477C118.574 58.1128 119.432 68.5204 111.83 69.6136C110.876 69.7349 109.91 69.7349 108.956 69.6136C108.103 69.5215 107.257 69.369 106.425 69.1574C92.8607 65.8001 91.711 52.9391 78.3611 52.414C78.3611 52.3796 68.1943 64.3367 74.5689 71.6366Z",
    fill: "var(--away-indicator)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M4.82518 61.9693C4.56779 62.6924 5.68315 66.2735 5.98343 69.5533C6.72128 77.4558 9.14073 89.585 20.38 94.2163C21.8043 94.7886 23.3167 95.1091 24.85 95.1633C25.8245 95.2069 26.801 95.1607 27.7671 95.0255C34.6308 94.1647 42.1722 89.146 45.913 81.321C51.0607 70.5174 53.1542 52.1901 53.1542 52.1901C48.1951 52.1901 35.9949 51.2346 31.5593 70.8962C31.2816 72.4298 30.6692 73.8827 29.7658 75.1508C28.8625 76.4189 27.6906 77.4706 26.3343 78.2305C25.4842 78.6807 24.5598 78.9724 23.606 79.0914C22.3104 79.258 20.994 79.07 19.7962 78.5472C18.5985 78.0243 17.564 77.1862 16.8023 76.1215C10.1445 67.1774 9.32089 57.5532 4.52489 56.6062C-0.271115 55.6593 6.07781 58.4485 4.82518 61.9693Z",
    fill: "var(--away-indicator)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M78.3611 52.3794C91.711 52.9045 92.8606 65.7655 106.425 69.1227C107.841 69.4995 109.302 69.6791 110.766 69.6565C111.393 74.2447 111.847 78.8588 112.285 83.4815C110.401 84.1397 108.419 84.4687 106.425 84.4543C95.3144 84.4543 80.935 78.8502 74.5689 71.5933C68.2028 64.3365 78.3611 52.3794 78.3611 52.3794Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M25.1846 78.7212C28.0502 77.7484 30.6241 75.0971 31.5592 70.8962C35.9606 51.2346 48.1951 52.1815 53.1455 52.1901C53.1455 52.1901 51.0779 70.5174 45.9129 81.3209C41.7432 90.0326 32.9062 95.2235 25.442 95.1719C26.163 89.7055 26.0763 84.1622 25.1846 78.7212Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M90.0293 91.9693C103.036 130.225 103.825 145.935 102.616 159.115C101.406 172.294 91.8224 185.852 79.3219 195.778C76.688 197.655 73.3934 198.98 75.4783 204.094C78.5069 211.531 70.5364 201.761 68.5459 196.897C66.5555 192.033 87.6613 183.571 82.7881 157.6C77.9149 131.628 64.8395 121.005 56.6632 102.626C48.4868 84.2475 90.0293 91.9693 90.0293 91.9693Z",
    fill: "#1C58D9"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M90.0293 91.9693C103.036 130.225 103.825 145.935 102.616 159.115C101.406 172.294 91.8224 185.852 79.3219 195.778C76.688 197.655 73.3934 198.98 75.4783 204.094C78.5069 211.531 70.5364 201.761 68.5459 196.897C66.5555 192.033 87.6613 183.571 82.7881 157.6C77.9149 131.628 64.8395 121.005 56.6632 102.626C48.4868 84.2475 90.0293 91.9693 90.0293 91.9693Z",
    fill: "black",
    fillOpacity: "0.16"
  }), /*#__PURE__*/react.createElement("mask", {
    id: "mask0_1032_50387",
    style: {
      maskType: 'alpha'
    },
    maskUnits: "userSpaceOnUse",
    x: "55",
    y: "90",
    width: "49",
    height: "117"
  }, /*#__PURE__*/react.createElement("path", {
    d: "M90.0293 91.9693C103.036 130.225 103.825 145.935 102.616 159.115C101.406 172.294 91.8224 185.852 79.3219 195.778C76.688 197.655 73.3934 198.98 75.4783 204.094C78.5069 211.531 70.5364 201.761 68.5459 196.897C66.5555 192.033 87.6613 183.571 82.7881 157.6C77.9149 131.628 64.8395 121.005 56.6632 102.626C48.4868 84.2475 90.0293 91.9693 90.0293 91.9693Z",
    fill: "#1452BD"
  })), /*#__PURE__*/react.createElement("g", {
    mask: "url(#mask0_1032_50387)"
  }, /*#__PURE__*/react.createElement("rect", {
    x: "50.2554",
    y: "83.8013",
    width: "34.4057",
    height: "96.183",
    fill: "url(#pattern0)"
  })), /*#__PURE__*/react.createElement("path", {
    d: "M73.3076 122.899C74.4573 125.094 75.7785 127.445 77.1684 129.872L76.6965 130.259C75.2466 127.677 73.8653 125.215 72.81 123.192C72.6384 122.865 73.1274 122.572 73.3076 122.899Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M80.3944 185.156C79.5364 186.111 78.6785 187.058 77.8634 188.005L77.5031 187.609C78.361 186.688 79.1675 185.758 79.9826 184.82L80.3944 185.156Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M74.8434 94.5176C76.4221 137.387 72.3296 157.109 53.8233 170.977C35.3171 184.846 20.972 185.345 20.4143 192.335C19.8566 199.325 18.1836 197.724 17.5316 190.553C16.6736 180.817 12.624 188.048 34.1331 171.399C55.6422 154.751 45.484 110.882 44.4287 97.8835C43.3734 84.8848 74.8434 94.5176 74.8434 94.5176Z",
    fill: "#1C58D9"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M20.1483 185.199C30.9415 179.861 42.1636 173.913 47.1741 162.257C52.7508 149.284 50.3743 134.71 50.0225 121.092C49.8166 112.879 50.0225 104.658 50.5545 96.4632C50.5545 96.093 51.155 96.093 51.1293 96.4632C50.6746 103.996 50.4343 111.536 50.563 119.086C50.6917 126.636 51.421 134.185 51.421 141.752C51.3781 148.303 50.666 154.949 48.3066 161.104C46.1355 166.701 42.5309 171.624 37.8566 175.376C32.6145 179.681 26.5144 182.719 20.4829 185.706C20.1054 185.861 19.8137 185.362 20.1483 185.199Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M84.0236 180.713C83.6632 181.195 83.2943 181.669 82.9168 182.142C82.3677 182.831 81.8015 183.503 81.2009 184.174L80.7891 183.847C81.7242 182.754 82.6337 181.634 83.4917 180.481L84.0236 180.713Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M86.8549 176.34C86.2157 177.477 85.5199 178.58 84.7701 179.646L84.2725 179.422C85.0017 178.372 85.6881 177.296 86.3144 176.177L86.8549 176.34Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M88.4592 171.339L89.0169 171.477C88.6073 172.745 88.0995 173.98 87.4983 175.17L86.9492 175.006C87.301 174.292 87.6442 173.568 87.9359 172.811C88.1332 172.355 88.2962 171.83 88.4592 171.339Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M90.0465 160.38C90.1601 161.525 90.2145 162.674 90.2095 163.824L89.6433 163.763C89.646 162.634 89.5887 161.504 89.4717 160.38H90.0465Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M88.914 154.277C89.0341 154.724 89.1457 155.138 89.2486 155.594C89.5232 156.756 89.7377 157.944 89.9007 159.132H89.3173C89.0866 157.553 88.7628 155.989 88.3478 154.449L88.914 154.277Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M87.4298 149.784C87.8416 150.86 88.2277 151.936 88.5537 152.995L87.9874 153.193C87.6614 152.108 87.2925 151.04 86.8978 149.982L87.4298 149.784Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M85.2678 144.808C85.8683 146.073 86.4346 147.339 86.9837 148.595L86.4689 148.793C85.9541 147.502 85.3879 146.211 84.7959 144.988L85.2678 144.808Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M79.871 134.598C80.6775 136.01 81.4754 137.445 82.2647 138.902L81.7843 139.255C81.0293 137.895 80.2142 136.449 79.3734 134.951L79.871 134.598Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M77.8205 131.034C78.2838 131.843 78.7557 132.661 79.2276 133.496L78.7385 133.866C78.2752 133.057 77.8034 132.231 77.3486 131.422L77.8205 131.034Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M82.8739 140.047C83.5088 141.235 84.118 142.426 84.7014 143.62L84.1952 143.801C83.9378 143.284 83.6804 142.768 83.4145 142.268C83.1485 141.769 82.7624 141.046 82.4106 140.392L82.8739 140.047Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M90.158 165.141C90.0693 166.859 89.8077 168.563 89.3773 170.228L88.8196 170.099C89.2452 168.457 89.5039 166.775 89.5918 165.081L90.158 165.141Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M77.0054 188.978C76.3534 189.718 75.7013 190.45 75.0664 191.207C74.9377 191.069 74.809 190.932 74.6718 190.803C75.3152 190.054 75.9758 189.313 76.6365 188.582C76.7566 188.711 76.8853 188.84 77.0054 188.978Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M73.831 191.792L74.2342 192.206C73.5221 193.067 72.8272 193.927 72.1579 194.831C71.9435 195.124 71.4458 194.831 71.6603 194.539C72.3553 193.6 73.1274 192.696 73.831 191.792Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M58.3706 52.0781C58.9108 52.9825 59.7345 53.6818 60.7128 54.0666C64.239 55.5214 68.7262 55.5214 70.6995 52.1383C86.915 52.1383 87.0608 52.8614 89.0684 76.5948C90.0808 88.526 90.6385 95.2578 88.9226 99.1918C79.9741 103.074 69.6871 104.357 60.3096 106.182C56.2257 106.974 52.1246 107.645 48.015 108.239C46.3505 107.835 45.3639 106.673 45.1322 104.185C44.3601 95.9206 43.871 96.1617 42.5584 82.1214C39.8215 53.9375 39.9845 52.0436 58.3706 52.0781Z",
    fill: "var(--center-channel-bg)"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M58.3706 52.0781C58.9108 52.9825 59.7345 53.6818 60.7128 54.0666C64.239 55.5214 68.7262 55.5214 70.6995 52.1383C86.915 52.1383 87.0608 52.8614 89.0684 76.5948C90.0808 88.526 90.6385 95.2578 88.9226 99.1918C79.9741 103.074 69.6871 104.357 60.3096 106.182C56.2257 106.974 52.1246 107.645 48.015 108.239C46.3505 107.835 45.3639 106.673 45.1322 104.185C44.3601 95.9206 43.871 96.1617 42.5584 82.1214C39.8215 53.9375 39.9845 52.0436 58.3706 52.0781Z",
    fill: "var(--center-channel-color)",
    fillOpacity: "0.08"
  }), /*#__PURE__*/react.createElement("mask", {
    id: "mask1_1032_50387",
    style: {
      maskType: 'alpha'
    },
    maskUnits: "userSpaceOnUse",
    x: "41",
    y: "52",
    width: "50",
    height: "57"
  }, /*#__PURE__*/react.createElement("path", {
    d: "M58.3706 52.0781C58.9108 52.9825 59.7345 53.6818 60.7128 54.0666C64.239 55.5214 68.7262 55.5214 70.6995 52.1383C86.915 52.1383 87.0608 52.8614 89.0684 76.5948C90.0808 88.526 90.6385 95.2578 88.9226 99.1918C79.9741 103.074 69.6871 104.357 60.3096 106.182C56.2257 106.974 52.1246 107.645 48.015 108.239C46.3505 107.835 45.3639 106.673 45.1322 104.185C44.3601 95.9206 43.871 96.1617 42.5584 82.1214C39.8215 53.9375 39.9845 52.0436 58.3706 52.0781Z",
    fill: "var(--center-channel-bg)"
  })), /*#__PURE__*/react.createElement("g", {
    mask: "url(#mask1_1032_50387)"
  }, /*#__PURE__*/react.createElement("rect", {
    opacity: "0.7",
    x: "40.2043",
    y: "58.5923",
    width: "42.9104",
    height: "50.8064",
    fill: "url(#pattern1)"
  })), /*#__PURE__*/react.createElement("path", {
    d: "M80.1714 97.3154C77.6915 92.7773 75.7644 87.9564 74.4317 82.9566C71.9522 72.8428 70.3844 62.5256 69.7472 52.13C86.9064 52.13 88.6223 51.5015 90.7072 75.6911C90.8101 76.8619 89.4546 78.9623 89.8492 80.1589C93.1695 89.5076 105.061 99.0974 104.083 101.249C102.719 104.331 97.5108 107.62 90.956 110.202C89.2401 110.641 87.4812 110.994 85.7224 111.27C84.2941 106.46 82.4362 101.79 80.1714 97.3154Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M42.3868 99.4761C41.8978 105.201 41.2543 110.099 41.6575 114.481C45.424 113.852 49.2076 113.456 53.0084 113.189C57.0966 104.476 59.5901 95.0952 60.3697 85.496C60.8673 79.7714 62.403 57.7856 62.0255 54.5058C61.7853 52.302 59.0484 54.1615 58.4907 52.061C39.9931 52.061 38.0112 53.4384 40.7224 81.6825C40.851 83.0943 42.7471 84.8848 42.8072 86.2192C42.9616 90.6426 42.8212 95.0715 42.3868 99.4761Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M70.9311 51.0281C72.2009 52.0525 71.9692 51.7684 71.1027 53.3781C70.2361 54.9879 69.7986 56.4428 69.0865 54.4886C68.3744 52.5345 68.28 52.8444 69.2066 51.6823C70.1332 50.5202 70.2876 50.5029 70.9311 51.0281Z",
    fill: "#1C58D9"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M60.3096 51.355C63.1838 52.6204 62.1285 52.3277 62.6776 54.2302C63.8702 58.3192 62.2057 56.1499 58.1304 53.7912C55.3592 52.1814 55.6852 51.9317 56.6547 50.9504C57.6242 49.969 57.3325 50.0121 60.1552 51.355C62.9779 52.6979 57.4269 50.1154 60.3096 51.355Z",
    fill: "#1C58D9"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M23.6403 79.2635C25.0318 84.4407 25.4422 89.8341 24.85 95.1632C25.8245 95.2068 26.801 95.1607 27.7671 95.0255C28.3164 89.4802 27.8629 83.8809 26.4287 78.4973C26.4101 78.4043 26.3783 78.3144 26.3343 78.2305C25.4842 78.6806 24.5598 78.9723 23.606 79.0913C23.6136 79.1494 23.6251 79.2069 23.6403 79.2635Z",
    fill: "#1C58D9"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M108.956 69.5789C109.482 74.3709 110.206 79.1285 111.127 83.8517C112.069 83.5963 112.985 83.2534 113.863 82.8273C113.006 78.4284 112.328 74.0209 111.83 69.5703C110.876 69.6944 109.911 69.6973 108.956 69.5789Z",
    fill: "#1C58D9"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M75.7528 198.524C74.7662 199.747 74.3887 201.382 75.4783 204.051C78.5069 211.489 70.5364 201.718 68.5459 196.854C68.0569 195.649 68.9749 194.229 70.5622 192.378C72.3381 194.392 74.0541 196.458 75.7528 198.524Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("path", {
    d: "M20.0197 181.609C20.1398 181.902 20.2599 182.186 20.3715 182.47C21.2294 184.415 22.0016 186.361 22.8596 188.289C22.1774 188.741 21.6021 189.338 21.1738 190.037C20.7455 190.736 20.4746 191.52 20.3801 192.335C19.831 199.316 18.1494 197.724 17.4973 190.553C16.9053 184.071 14.932 185.104 20.0197 181.609Z",
    fill: "#1E325C"
  }), /*#__PURE__*/react.createElement("ellipse", {
    cx: "52.3383",
    cy: "206.794",
    rx: "33.75",
    ry: "1.76471",
    fill: "black",
    fillOpacity: "0.12"
  }), /*#__PURE__*/react.createElement("defs", null, /*#__PURE__*/react.createElement("pattern", {
    id: "pattern0",
    patternContentUnits: "objectBoundingBox",
    width: "1",
    height: "1"
  }, /*#__PURE__*/react.createElement("use", {
    xlinkHref: "#image0_1032_50387",
    transform: "scale(0.011236 0.00403226)"
  })), /*#__PURE__*/react.createElement("pattern", {
    id: "pattern1",
    patternContentUnits: "objectBoundingBox",
    width: "1",
    height: "1"
  }, /*#__PURE__*/react.createElement("use", {
    xlinkHref: "#image1_1032_50387",
    transform: "scale(0.00900901 0.00763359)"
  })), /*#__PURE__*/react.createElement("image", {
    id: "image0_1032_50387",
    width: "89",
    height: "248",
    xlinkHref: "data:image/png;base64,"
  }), /*#__PURE__*/react.createElement("image", {
    id: "image1_1032_50387",
    width: "111",
    height: "131",
    xlinkHref: "data:image/png;base64,"
  })));
};

Svg.propTypes = {
  width: (prop_types_default()).number,
  height: (prop_types_default()).number
};
/* harmony default export */ const man_with_mailbox_svg = (Svg);
// EXTERNAL MODULE: ./components/header_footer_route/content_layouts/column.tsx
var column = __webpack_require__(71284);
// EXTERNAL MODULE: ./components/save_button.tsx
var save_button = __webpack_require__(58657);
// EXTERNAL MODULE: ./packages/mattermost-redux/src/actions/users.ts
var users = __webpack_require__(33383);
;// CONCATENATED MODULE: ./components/should_verify_email/should_verify_email.tsx





// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.










var ResendStatus;

(function (ResendStatus) {
  ResendStatus["PENDING"] = "pending";
  ResendStatus["SUCCESS"] = "success";
  ResendStatus["FAILURE"] = "failure";
})(ResendStatus || (ResendStatus = {}));

const ShouldVerifyEmail = () => {
  var _params$get;

  const {
    formatMessage
  } = (0,useIntl/* default */.Z)();
  const dispatch = (0,es.useDispatch)();
  const history = (0,react_router/* useHistory */.k6)();
  const {
    search
  } = (0,react_router/* useLocation */.TH)();
  const params = new URLSearchParams(search);
  const email = (_params$get = params.get('email')) !== null && _params$get !== void 0 ? _params$get : '';
  const [resendStatus, setResendStatus] = (0,react.useState)(ResendStatus.PENDING);
  const [isWaiting, setIsWaiting] = (0,react.useState)(false);
  const handleReturnButtonOnClick = (0,react.useCallback)(() => {
    history.push('/');
  }, [history]);

  const handleResendButtonOnClick = async () => {
    if (email) {
      setIsWaiting(true);
      setResendStatus(ResendStatus.PENDING);
      const {
        error
      } = await dispatch((0,users/* sendVerificationEmail */.zk)(email));

      if (error) {
        setResendStatus(ResendStatus.FAILURE);
        setIsWaiting(false);
        return;
      }

      setResendStatus(ResendStatus.SUCCESS);
      setIsWaiting(false);
    }
  };

  return /*#__PURE__*/react.createElement("div", {
    className: "should-verify-body"
  }, /*#__PURE__*/react.createElement("div", {
    className: "should-verify-body-content"
  }, /*#__PURE__*/react.createElement(column/* default */.Z, {
    title: formatMessage({
      id: 'email_verify.almost',
      defaultMessage: 'You’re almost done!'
    }),
    message: formatMessage({
      id: 'email_verify.notVerifiedBody',
      defaultMessage: 'Please verify your email address. Check your inbox for an email.'
    }),
    SVGElement: /*#__PURE__*/react.createElement(man_with_mailbox_svg, {
      width: 284
    }),
    extraContent: /*#__PURE__*/react.createElement("div", {
      className: "should-verify-body-content-extra"
    }, /*#__PURE__*/react.createElement("div", {
      className: "should-verify-body-content-buttons"
    }, /*#__PURE__*/react.createElement(save_button/* default */.Z, {
      extraClasses: "should-verify-body-content-button-resend large",
      saving: isWaiting,
      disabled: !email,
      onClick: handleResendButtonOnClick,
      defaultMessage: formatMessage({
        id: 'email_verify.resend',
        defaultMessage: 'Resend Email'
      }),
      savingMessage: formatMessage({
        id: 'email_verify.sending',
        defaultMessage: 'Sending email…'
      })
    }), /*#__PURE__*/react.createElement("button", {
      className: "should-verify-body-content-button-return",
      onClick: handleReturnButtonOnClick
    }, formatMessage({
      id: 'email_verify.return',
      defaultMessage: 'Return to log in'
    }))), /*#__PURE__*/react.createElement("div", {
      className: classnames_default()('should-verify-body-content-message', resendStatus)
    }, /*#__PURE__*/react.createElement("i", {
      className: classnames_default()('should-verify-body-content-message-icon', 'icon', 'icon-12', {
        'icon-check': resendStatus === ResendStatus.SUCCESS,
        'icon-alert-outline': resendStatus === ResendStatus.FAILURE
      })
    }), /*#__PURE__*/react.createElement("span", {
      className: "should-verify-body-content-message-label"
    }, resendStatus === ResendStatus.SUCCESS ? formatMessage({
      id: 'email_verify.sent',
      defaultMessage: 'Verification email sent'
    }) : resendStatus === ResendStatus.FAILURE && formatMessage({
      id: 'email_verify.failed',
      defaultMessage: 'Failed to send verification email'
    }) || '')))
  })));
};

/* harmony default export */ const should_verify_email = (ShouldVerifyEmail);

/***/ })

}]);
//# sourceMappingURL=741.200e282f21d99ff9c26c.js.map